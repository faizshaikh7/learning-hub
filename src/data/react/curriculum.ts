...
export const REACT_CURRICULUM: CurriculumTopic[] = [
  ...
  /* â”€â”€ PHASE 0: JS & TS Essentials â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
  {
    ...
    eli5: 'Before you can build with React you need to speak modern JavaScript â€” things like unpacking boxes (destructuring), copying lists (spread), asking "is this thing here?" (optional chaining), and short ways to write functions.',
    ...
    technicalDeep: 'Destructuring works on arrays and objects: `const { name, age = 0 } = user` pulls fields out and provides defaults. Spread copies shallowly: `{ ...obj, key: newVal }` is how React state updates are written. Optional chaining short-circuits: `user?.profile?.avatar` returns undefined instead of throwing. Nullish coalescing `??` only falls back on null/undefined, not 0 or "". Template literals `` `Hello ${name}` `` replace string concat. Arrow functions `(x) => x * 2` capture the surrounding `this` â€” critical for event handlers.',
    whatBreaks: 'Mutating objects/arrays directly instead of spreading them is the #1 React bug â€” it fools the equality check and state updates are silently ignored.',
    efficientWay: {
      ...
      approaches: [
        ...
        { name: 'Read the MDN page per feature', verdict: 'ok', reason: 'Good reference but passive â€” you need to type it, not just read it.' },
      ...
      recommendation: 'Spend one session (2h) running each feature in the browser console on real data. The goal is recognition, not mastery â€” you will deepen understanding while writing React.'
    },
    commonMistakes: [
      'Mutating state objects directly (`state.count++`) â€” always return a new reference.',
      'Confusing `||` and `??` â€” `||` falls back on ANY falsy value including 0 and "", which breaks number inputs.',
      'Arrow functions vs function declarations in class methods â€” in React hooks this rarely matters, but know the difference.'
    ],
    seniorNotes: 'In code review, spread misuse (`[...arr]` at the wrong level) is the most common subtle bug. The team\'s ESLint config usually catches direct mutation â€” know what "no-mutating-props" and "prefer-const" rules enforce.',
    interviewQuestions: [
...
const url = \`/api/users/\${userId}/posts?page=\${page}\`;`
...

  {
    ...
    explanation: 'Closures allow inner functions to access outer variables even after the outer function returns. In React, closures are everywhere: event handlers capture state values, useEffect callbacks capture deps, and stale closures are a common bug. Arrow functions (`=>`) inherit `this` from their surrounding scope â€” critical for object methods used as callbacks.',
    technicalDeep: 'Each call to a function creates a new closure scope. In React: `const handleClick = () => setCount(count + 1)` â€” this handler "closes over" the current `count` value. If `count` is stale (the component re-rendered), you get a stale closure bug. Fix: use the functional update form `setCount(c => c + 1)`. Arrow functions do NOT have their own `this`, `arguments`, or `prototype`. IIFE pattern `(() => { ... })()` is how the existing tutors are structured.',
    whatBreaks: 'Stale closures in useEffect: capturing a value in the dependency array but forgetting to list it â€” the effect runs with an outdated snapshot of the value.',
    efficientWay: {
      ...
      approaches: [
        ...
        { name: 'Read Kyle Simpson\'s "You Don\'t Know JS" closures chapter', verdict: 'ok', reason: 'Deep but slow â€” good for understanding, not for shipping.' },
    ...
    seniorNotes: 'The functional update pattern `setState(prev => ...)` is the closure-safe form â€” it receives the latest state rather than closing over a potentially stale snapshot. Always use it when new state depends on old state.',
    interviewQuestions: [
    ...
    codeExamples: [
      {
        ...
        code: `// BUG: stale closure â€” count is captured once
...
// FIX: functional update â€” always uses latest value
useEffect(() => {
  const id = setInterval(() => {
...

  {
    ...
    eli5: 'React uses arrays to show lists of things. The magic tricks â€” map, filter, find, reduce â€” let you transform, search, and summarize those lists without writing big loops.',
    ...
    explanation: '`map` turns every element into a new element â€” this is how you render lists in JSX. `filter` keeps elements that pass a test â€” used for search, hiding completed items. `find`/`findIndex` locate one item. `reduce` aggregates. `some`/`every` check conditions. None of these mutate the original array, which is exactly what React requires.',
    technicalDeep: '`[].map(fn)` returns a NEW array of the same length. `[].filter(fn)` returns a subset. Both are O(n). For React lists, every mapped element needs a stable `key` prop â€” React uses it to reconcile. `reduce` with object accumulator is the pattern for grouping data for charts/summaries. `flatMap` maps then flattens one level â€” useful when one item produces multiple elements. Array spreading is O(n) so avoid it inside tight render loops.',
    whatBreaks: 'Returning `undefined` from a map (forgetting `return` in a multi-line arrow) gives an array of undefined â€” JSX renders nothing. Forgetting `key` on mapped elements causes React to re-render everything instead of patching.',
    efficientWay: {
      title: 'Mastering array methods',
      approaches: [
        { name: 'Practice on real API data (JSONPlaceholder)', verdict: 'best', reason: 'Filtering todos, mapping user names to <li> tags â€” this is literally what you will do in React.' },
        { name: 'LeetCode array problems', verdict: 'ok', reason: 'Builds skill but the problems are contrived â€” context differs from React usage.' },
    ...
    commonMistakes: [
      'Mutating inside map/filter (`item.checked = true; return item`) â€” creates the same reference, React skips the update.',
      'Using the array index as the key when the list can reorder â€” causes wrong diffs.',
...
const doneCount = todos.reduce((acc, t) => acc + (t.done ? 1 : 0), 0);`
...

  {
    ...
    explanation: 'All data fetching in React uses Promises. `async/await` is syntax sugar over `.then/.catch` chains. In React components, you cannot use `await` directly inside render â€” you use it inside `useEffect` callbacks or event handlers. Network errors must be caught with try/catch or `.catch()`.',
    technicalDeep: 'The JS event loop: synchronous code runs first, then microtasks (Promises), then macrotasks (setTimeout). `await` suspends the async function at that point and yields control back to the event loop â€” the UI thread keeps painting. `Promise.all([a,b,c])` runs all concurrently and resolves when all complete. `Promise.allSettled` resolves even if some reject. `Promise.race` resolves/rejects with the first to finish. `fetch()` returns a Promise that resolves to a `Response` â€” you must `await response.json()` to get the body.',
    whatBreaks: 'Forgetting to `await response.json()` returns a Response object, not data. Uncaught Promise rejections crash silently in React unless an Error Boundary catches them. Creating async functions inside `useEffect` directly returns a Promise which React warns about â€” wrap in an immediately-invoked async function instead.',
    efficientWay: {
      title: 'Getting comfortable with async',
      approaches: [
        { name: 'Build a simple fetch-and-display component', verdict: 'best', reason: 'Loading/error/success states are the core pattern â€” you will write it hundreds of times.' },
    ...
    commonMistakes: [
      '`useEffect(async () => { ... })` â€” useEffect cannot accept an async function directly; wrap the call inside.',
      'Not handling the component-unmounted case â€” setState on an unmounted component causes memory leaks.',
...

  {
    ...
    analogy: 'A module is a LEGO set box. Each box exports specific pieces. Other boxes can import exactly the pieces they need â€” nobody dumps all the pieces into one pile.',
    explanation: 'React apps are built from dozens of files, each a module. Named exports (`export const`) let you export multiple things from one file. Default exports (`export default`) export one main thing. `import` statements tell bundlers (Vite/Webpack) to link the files â€” tree-shaking removes unused imports from the final bundle.',
    technicalDeep: 'ES modules are static: `import` statements are hoisted and analyzed at compile time, which enables tree-shaking. Dynamic `import()` is a function that returns a Promise â€” this is how React.lazy works for code splitting. Re-exports `export { X } from "./y"` create barrel files (index.ts). Barrels are convenient but can break tree-shaking if the bundler cannot statically analyze them â€” some teams ban barrel files for this reason. `import type { User }` imports TypeScript types only; erased at build time, no runtime cost.',
    whatBreaks: 'Circular imports (A imports B, B imports A) cause undefined values at runtime â€” one module loads before its dependency. Barrel files with side-effect imports can cause unexpected code to run. Mixing CommonJS (`require`) with ESM (`import`) breaks in Node without configuration.',
    efficientWay: {
      ...
      approaches: [
        ...
        { name: 'Memorize named vs default export syntax', verdict: 'ok', reason: 'Necessary but not sufficient â€” you need to understand what bundlers do with it.' },
    ...
    commonMistakes: [
      ...
      'Using `export default` for utilities â€” named exports are easier to refactor and auto-import.'
    ],
    seniorNotes: 'Dynamic `import()` is the foundation of React.lazy and Next.js `next/dynamic` â€” both code-split at that boundary. The browser downloads that chunk only when the component is first rendered, improving initial load.',
    interviewQuestions: [
    ...
    codeExamples: [
      {
        ...
        code: `// utils.ts â€” named exports (preferred for utilities)
export function formatDate(d: Date) { return d.toLocaleDateString(); }
export const API_URL = 'https://api.example.com';

// UserCard.tsx â€” default export (ok for components)
export default function UserCard({ name }: { name: string }) {
  return <div>{name}</div>;
}

// page.tsx â€” importing both styles
...
const HeavyChart = React.lazy(() => import('./HeavyChart'));`
...

  {
    ...
    explanation: 'TypeScript catches type errors at development time, not runtime. `type` creates aliases for any type expression. `interface` describes object shapes and can be extended with `extends`. In React, you type props with interfaces, state with generics, and API responses with mapped types. TypeScript is compiled away â€” zero runtime cost.',
    technicalDeep: 'Type vs Interface: interfaces support `extends` and declaration merging (two files can add to the same interface). Types support unions, intersections, and mapped types. For React props, both work; teams pick one convention. Union types `string | null` model nullable data. Intersection `A & B` merges shapes. Literal types `\'asc\' | \'desc\'` restrict to exact string values â€” perfect for enums. `as const` makes an object\'s values literal types. The `satisfies` operator (TS 4.9) checks a type without widening, preserving inference.',
    whatBreaks: '`any` defeats the purpose of TypeScript â€” treat it as a red flag in code review. `unknown` is the safe alternative: you must narrow before using. Type assertions `as SomeType` can lie to the compiler and cause runtime crashes.',
    efficientWay: {
      title: 'Learning TypeScript for React',
      approaches: [
        { name: 'Type every React component prop from day one', verdict: 'best', reason: 'Props are the most common TS usage in React â€” instant feedback, always in context.' },
        ...
        { name: 'Skip TypeScript until you are comfortable with React', verdict: 'weak', reason: 'Adding TS later to an existing codebase is painful â€” it\'s easier to start typed.' }
      ],
    ...
    seniorNotes: 'In code review, `// @ts-ignore` and `as any` are red flags that warrant a comment explaining WHY. Prefer `// @ts-expect-error` (fails if the error disappears) over `// @ts-ignore` (silently continues). Typed API response shapes are worth the effort â€” they catch backend contract breaks at compile time.',
    interviewQuestions: [
...
// as const â€” values become literal types
...
type Role = typeof ROLES[number]; // 'admin' | 'user' | 'guest'`
...

  {
    ...
    technicalDeep: 'Syntax: `function identity<T>(val: T): T { return val; }`. TypeScript infers `T` from the argument most of the time â€” you rarely need to annotate explicitly. Constraints `<T extends object>` restrict what types can fill the blank. Multiple type params `<T, K extends keyof T>` are common in utility functions. `useState<T>` in React is `function useState<S>(initial: S | (() => S)): [S, Dispatch<SetStateAction<S>>]`. Generic components in TSX need a workaround: `<T,>` (trailing comma) or `<T extends unknown>` to avoid JSX parser ambiguity with `<T>`.',
    whatBreaks: 'Using `any` instead of a generic defeats type inference. Over-constraining generics makes them too rigid. In `.tsx` files, `<T>` is interpreted as a JSX tag â€” always write `<T,>` or `<T extends unknown>` for generic arrow functions.',
    efficientWay: {
      title: 'Learning generics',
      approaches: [
        { name: 'Read how useState is typed in the React source', verdict: 'best', reason: 'It is a generic you use every day â€” understanding it makes all others easier.' },
    ...
    commonMistakes: [
      'Writing `<T>` instead of `<T,>` in `.tsx` arrow functions â€” TS parses it as a JSX tag.',
      'Defaulting to `any` when generics feel complicated â€” `unknown` with narrowing is almost always better.',
      'Over-constraining: `<T extends string>` when you just need `T` â€” adds friction with no benefit.'
    ],
    ...
    codeExamples: [
      {
        ...
        code: `// A generic hook â€” T is inferred from the call site
...
//    user is User | null â€” fully typed!`
      }
    ]
  },

  {
    ...
    technicalDeep: 'Key built-in utilities: `Partial<T>` â€” all keys optional. `Required<T>` â€” all keys required. `Readonly<T>` â€” immutable. `Pick<T, "a"|"b">` â€” keep those keys. `Omit<T, "password">` â€” exclude those keys. `Record<string, number>` â€” object with that key/value type. `Extract<T, U>` â€” intersection of unions. `Exclude<T, U>` â€” remove U from T union. `NonNullable<T>` â€” removes null/undefined. `ReturnType<typeof fn>` â€” the return type of a function. `Parameters<typeof fn>` â€” tuple of param types. These are implemented with mapped and conditional types under the hood.',
    ...
    efficientWay: {
      ...
      recommendation: 'Learn Partial, Pick, Omit, and Record first â€” they appear in 90% of React code. Learn ReturnType and Parameters when you start writing utility hooks.'
    },
    commonMistakes: [
      'Using `Partial<User>` for form values when only some fields are present â€” creates confusing optionality.',
      'Forgetting `ReturnType<typeof hook>` when you need to type the return value without duplicating it.',
      'Using `Record<string, any>` â€” defeats type safety; use `Record<string, User>` or a mapped type.'
    ],
...
type AuthContext = ReturnType<typeof useAuth>;`
...

  {
    ...
    eli5: 'Narrowing is TypeScript checking your "is this thing what I think it is?" tests. If you check `if (typeof x === "string")` TypeScript knows x is a string inside the block â€” it gets smarter as you write more checks.',
    analogy: 'Narrowing is like a bouncer who checks IDs. After the check, TypeScript knows exactly who got in â€” it stops treating everyone as a suspicious `unknown`.',
    ...
    technicalDeep: 'Type guards: `typeof x === "string"` narrows primitives. `x instanceof Date` narrows class instances. `"field" in obj` narrows to types that have that field. `Array.isArray(x)` narrows to array. Custom type predicates: `function isUser(x: unknown): x is User { return ... }` â€” the `is` keyword tells TS what the true branch knows. Discriminated unions: `{ kind: "circle"; radius: number } | { kind: "rect"; width: number }` â€” switch on `kind` and TypeScript knows which branch you\'re in. Exhaustive check pattern: `const _: never = state` after a switch â€” TS errors if you miss a case.',
    whatBreaks: '`as SomeType` is NOT narrowing â€” it is an assertion that bypasses the check and can lie. Use `is` predicates instead. Forgetting to narrow after `JSON.parse()` (returns `any`) means you lose all type safety.',
    efficientWay: {
      ...
      approaches: [
        ...
        { name: 'Read the TS handbook narrowing chapter', verdict: 'ok', reason: 'Good reference, with good examples â€” solid 30 min read.' },
    ...
    commonMistakes: [
      ...
      'Not handling all union members â€” TypeScript\'s `never` check catches this at compile time.',
      'Treating `null` and `undefined` as the same â€” `x != null` narrows both, but `x !== null` only narrows null.'
    ],
...
function UserView() {
...

  {
    ...
    eli5: 'The DOM is the browser\'s model of your webpage as a tree of boxes. Events are things that happen (click, type, scroll). React replaces direct DOM manipulation with a Virtual DOM â€” understanding what React abstracts away helps you know when you need to go around it.',
    ...
    explanation: 'React wraps native DOM events with `SyntheticEvent` â€” a cross-browser wrapper. You almost never touch the real DOM in React, but you need to know it exists for: third-party libraries that manipulate DOM directly, `useRef` for DOM access, `document.addEventListener` for global events, and `getBoundingClientRect()` for positioning.',
    ...
    whatBreaks: '`e.target.value` works for input events; `e.target` is typed as `EventTarget` in TypeScript which has no `.value` â€” cast to `HTMLInputElement`. Attaching event listeners to `document` inside useEffect without removing them in cleanup causes memory leaks.',
    efficientWay: {
    ...
    seniorNotes: 'React\'s event system means you never call `removeEventListener` for JSX handlers â€” React handles it. But `useEffect`-attached `document` / `window` listeners need manual cleanup. This distinction is one of the top causes of memory leaks in React apps.',
    interviewQuestions: [
  ...
  function handleChange(e: React.ChangeEvent<HTMLInputElement>) {
    // e.target IS HTMLInputElement here â€” React types it correctly
  ...
  /* â”€â”€ PHASE 1: React Foundations â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
  {
    ...
    analogy: 'The real DOM is an Etch A Sketch â€” to change one line you have to shake the whole thing. React\'s Virtual DOM is a rough sketch on paper. You change the sketch, then transfer only the differences to the Etch A Sketch.',
    ...
    efficientWay: {
      ...
      approaches: [
        ...
        { name: 'Read the React Fiber architecture blog post', verdict: 'ok', reason: 'Deep and accurate, but dense â€” better after you have hands-on experience.' },
    ...
    commonMistakes: [
      'Thinking React updates the whole page on every state change â€” it only updates the changed subtree.',
      'Nesting component definitions inside render functions â€” creates a new component type every render, forcing unmount/remount.',
      'Treating the Virtual DOM as a performance win in all cases â€” for very simple UIs, direct DOM manipulation is faster. React wins at scale.'
    ],
    seniorNotes: 'In interviews: React\'s reconciliation is O(n) (linear scan of both trees), not the theoretical O(nÂ³) of arbitrary tree diffing â€” it makes two assumptions: elements of different types produce different trees, and the key prop is stable identity. Both are your responsibility as the developer.',
    interviewQuestions: [
    ...
    codeExamples: [
      {
        ...
        code: `// SAME key â€” React UPDATES the existing Input (keeps focus, value, state)
function App() {
...
// DIFFERENT key â€” React UNMOUNTS old, MOUNTS new (resets all state)
...
<UserForm key={userId} userId={userId} />`
...

  {
    ...
    explanation: 'Vite is the de-facto React build tool in 2024+. It uses ES module native imports in development (instant HMR â€” hot module replacement) and Rollup for production builds. Create a TypeScript React project in seconds: `npm create vite@latest my-app -- --template react-ts`. No configuration needed to start.',
    technicalDeep: 'Vite\'s dev server serves source files as ES modules â€” the browser requests each file, Vite transforms on demand (no bundling). This is why HMR is instant regardless of app size. In production, Vite uses Rollup with code splitting at dynamic `import()` boundaries. Key config: `vite.config.ts` for aliases (`@/` â†’ `./src/`), proxy (for dev API calls to avoid CORS), and environment variables. `.env` files: `VITE_` prefix makes vars available in client code via `import.meta.env.VITE_API_URL`. Non-prefixed vars stay server-only. `tsconfig.json` `paths` must match Vite aliases for TypeScript to resolve correctly.',
    whatBreaks: '`process.env.REACT_APP_*` (Create React App convention) does not work in Vite â€” use `import.meta.env.VITE_*`. The `public/` folder is served as-is; `src/assets/` goes through the bundler (gets hashed for cache busting).',
    efficientWay: {
    ...
    commonMistakes: [
      'Using `process.env.REACT_APP_X` from old CRA habits â€” Vite uses `import.meta.env.VITE_X`.',
      'Not configuring `@/` path aliases â€” leads to relative import hell (`../../../../components/`).',
      'Committing `.env` files â€” use `.env.example` as a template, `.env.local` for real values.'
    ],
    seniorNotes: 'For monorepos, Turborepo + Vite workspaces is the current best practice. Vite\'s `preview` command (`npm run preview`) serves the production build locally â€” always run this before deploying to catch asset and routing issues the dev server hides.',
    interviewQuestions: [
...
# VITE_API_URL=http://localhost:3001`
...

  {
    ...
    title: 'JSX & TSX',
    eli5: 'JSX lets you write HTML-looking code inside JavaScript. It\'s not real HTML â€” it gets compiled into `React.createElement()` calls. TSX is the same thing but in TypeScript files.',
    ...
    explanation: 'JSX is syntactic sugar compiled by Babel/SWC to `React.createElement(type, props, ...children)` calls. Rules: class â†’ className, for â†’ htmlFor, all tags must close (`<br />`), one root element (or use `<>` fragment), expressions in `{}`. TSX adds TypeScript â€” all JSX rules plus prop types.',
    technicalDeep: 'Modern React (17+) uses the new JSX transform â€” no more `import React from "react"` at the top of every file. The transform imports `jsx` from `react/jsx-runtime` automatically. JSX expressions can contain any JS expression; statements (if, for) cannot appear directly â€” use ternary `? :` or `&&` for conditionals, `array.map()` for lists. String props don\'t need `{}`: `<Btn label="Click" />`. Non-string props always need `{}`: `<Btn count={5} />`. Spreading props: `<Comp {...props} />` â€” equivalent to listing every prop individually. Self-closing tags: any component without children should self-close: `<Avatar />` not `<Avatar></Avatar>`.',
    whatBreaks: '`class` instead of `className` silently adds an unknown DOM attribute â€” no error, just missing styles. Returning adjacent elements without a wrapper throws "JSX must have one parent element". Falsy values: `0` renders as the text "0" â€” use `condition !== 0 && <X>` or `!!condition && <X>`.',
    efficientWay: {
      title: 'Learning JSX',
      approaches: [
        ...
        { name: 'Use a template engine instead of JSX', verdict: 'weak', reason: 'JSX is the universal React convention â€” avoiding it adds friction with the entire ecosystem.' }
      ],
    ...
    commonMistakes: [
      '`class` instead of `className` â€” 100% of beginners hit this at least once.',
      'Rendering `{condition && <X>}` when condition can be `0` â€” renders the literal "0".',
      'Multi-line JSX without parentheses after `return` â€” ASI (automatic semicolon insertion) inserts a semicolon after `return` on its own line, returning undefined.'
    ],
    seniorNotes: 'The new JSX transform (React 17+) means you no longer need `import React from "react"` â€” but many existing codebases still have it. ESLint rule `react/react-in-jsx-scope` should be turned off for the new transform. When you see it in old code, it\'s not wrong â€” just vestigial.',
    interviewQuestions: [
...
// Fragment shorthand â€” no extra DOM node
function Wrapper() {
  return (
    <>
      <h1>Title</h1>
...

  {
    ...
    eli5: 'A component is a reusable piece of UI â€” like a LEGO brick. A functional component is just a JavaScript function that returns JSX. You call it like an HTML tag: `<MyButton />` and React puts it on the screen.',
    ...
    explanation: 'Functional components are the standard React component type since hooks arrived in 2019. They are plain functions: receive props as an argument, return JSX (or null). Component names MUST start with a capital letter â€” lowercase names are treated as DOM elements. A component must be pure for the same props: same output every time during render.',
    technicalDeep: 'React calls your component function on every render. The function must be pure during render â€” no side effects, no random values, no mutations. Hooks (`useState`, `useEffect`) let you opt into React features from a functional component. Component identity is defined by the function reference â€” if you define a component inside another component\'s render, React sees a new type every render and unmounts/remounts instead of updating. `React.FC<Props>` (or `React.FunctionComponent`) is the TypeScript annotation â€” but many teams now prefer the simpler `function Component(props: Props): JSX.Element` form, which avoids the implicit `children` prop that `React.FC` used to include.',
    whatBreaks: 'Defining a component inside another component\'s body causes it to be treated as a new type every render â€” its state resets on every parent render. Always define components at module scope.',
    efficientWay: {
      title: 'Writing good functional components',
      approaches: [
        { name: 'Write small, single-responsibility components from day one', verdict: 'best', reason: 'Breaking UI into small pieces is the skill â€” you will refactor if you start big.' },
    ...
    commonMistakes: [
      'Lowercase component names â€” `<myButton />` is treated as a DOM element, not a component.',
      'Defining components inside other components â€” causes state loss on every parent render.',
      'Side effects directly in the function body instead of useEffect â€” runs during render, breaks Strict Mode double-invoke.'
    ],
    seniorNotes: 'React Strict Mode (on by default in Vite) calls your component function twice in development to surface impure renders. If your component breaks on double-invocation, it has a side effect in the body â€” this is intentional. Fix the component, not Strict Mode.',
    interviewQuestions: [
...
// Function declaration â€” preferred over arrow function for components
function UserCard({ name, email, avatarUrl }: UserCardProps): JSX.Element {
  // âœ… Hooks here â€” always at the top level
  const [expanded, setExpanded] = useState(false);

  // âœ… Derived values â€” computed from props/state
  const initials = name.split(' ').map(n => n[0]).join('');

  // âœ… Event handlers â€” defined in component body
  function handleToggle() { setExpanded(e => !e); }

  // âœ… Return JSX â€” the only required output
  return (
...
export default UserCard;`
...

  {
    ...
    technicalDeep: 'Props are plain objects received as the first argument. Destructuring is idiomatic: `function Btn({ label, onClick }: BtnProps)`. Default values: `function Btn({ label = "Click" })`. Spread props `<Child {...parentProps} />` passes all parent props â€” useful for wrappers but can leak unwanted props to DOM elements (TypeScript catches this). `children` is a special prop: `React.ReactNode` type, passed between opening/closing tags. Prop drilling (passing props many levels deep) is a smell â€” Context or state management is the fix. Never mutate props â€” React relies on props being immutable to optimize renders.',
    whatBreaks: 'Mutating props directly (`props.user.name = "new"`) causes subtle bugs â€” React may not re-render because the reference didn\'t change. Spreading all props onto a DOM element passes unknown HTML attributes â€” TypeScript\'s `HTMLDivElement` props type catches this.',
    efficientWay: {
      title: 'Working with props',
      approaches: [
        { name: 'Type every prop interface before writing the component body', verdict: 'best', reason: 'The props interface IS the component\'s contract â€” defining it first forces you to think about the API.' },
    ...
    commonMistakes: [
      ...
      'Passing a new object literal as a prop on every render: `<Comp style={{ margin: 0 }} />` â€” creates a new reference each time, breaks memoization.',
      'Prop drilling 3+ levels deep instead of lifting state or using Context.'
    ],
    seniorNotes: '`children` as `React.ReactNode` is flexible but untyped. For more control, use explicit render props or named slot props: `header: ReactNode; footer?: ReactNode`. The "component API design" skill â€” knowing which things should be props vs internal state â€” separates good component libraries from hard-to-use ones.',
    interviewQuestions: [
...
// Usage â€” parent passes callback to receive "close" signal
function App() {
...

  {
    ...
    title: 'useState â€” Local Component State',
    ...
    explanation: '`const [value, setValue] = useState(initial)` returns the current value and a setter. Calling the setter schedules a re-render with the new value. State is local to each instance â€” multiple `<Counter />` components have independent counts. State updates are batched in React 18, so multiple `setState` calls in one event handler trigger one render.',
    technicalDeep: 'React stores state in a linked list tied to each Fiber (component instance). Each `useState` call claims a slot in that list â€” which is why hooks must be called in the same order every render (no conditionals, loops, or early returns before hooks). State updates are not immediate â€” the setter schedules an update; the new value is available on the NEXT render. Batching: in React 18, all state updates (even in async callbacks) are batched by default. The functional updater `setValue(v => v + 1)` is closure-safe and should be used whenever new state depends on old state. `useState(fn)` lazy initialization: if the initial value is expensive to compute, pass a function â€” it runs once on mount, not every render.',
    whatBreaks: 'Reading state immediately after calling the setter returns the OLD value â€” state has not updated yet. Storing derived data in state that should be computed from other state/props causes sync bugs.',
    efficientWay: {
      ...
      approaches: [
        ...
        { name: 'Store everything in state including derived data', verdict: 'weak', reason: 'Derived state must be kept in sync manually â€” the source of countless bugs.' }
      ],
    ...
    commonMistakes: [
      'Reading the new state value immediately after calling the setter â€” it\'s still the old value in the current render.',
      'Storing computed/derived data in state instead of calculating it from existing state.',
      'Directly mutating state objects: `state.count++` â€” React detects changes by reference, so mutation is invisible.'
    ],
...
// Object state â€” spread to update immutably
const [user, setUser] = useState({ name: '', email: '' });
function updateEmail(email: string) {
  setUser(prev => ({ ...prev, email })); // functional update â€” closure-safe
}

// Derived value â€” NOT in state, computed from state
const isValid = name.length >= 3; // re-computed every render, always in sync

// Lazy init â€” computeExpensiveDefault runs ONCE, not every render
...
// âœ… Instead: const doubled = count * 2;`
...

  {
    ...
    eli5: 'Events are things the user does â€” clicking, typing, submitting a form. React lets you attach functions to these events. When the event happens, your function runs.',
    ...
    technicalDeep: 'React wraps native events in `SyntheticEvent` â€” normalized across browsers. Event delegation: React attaches one listener at the root (React 17+), not on each element. `e.preventDefault()` stops default browser behavior (form navigation, link following). `e.stopPropagation()` stops bubbling â€” use sparingly as it breaks higher-level event listeners. For inline functions `onClick={() => doThing(id)}` â€” creates a new function on every render. For lists this causes every item to re-render if the parent re-renders. Fix: `useCallback` + `React.memo` on list items (measure first â€” premature optimization is real).',
    whatBreaks: '`onClick={handleClick()}` with parentheses calls the function during render, not on click. Forgetting `e.preventDefault()` on form submit causes the page to reload. Accessing `e.target.value` asynchronously (after a setTimeout) in React 16 returned null because of event pooling â€” this is fixed in React 17+.',
    efficientWay: {
      title: 'Event handling patterns',
      approaches: [
        { name: 'Always type event parameters for TypeScript hints', verdict: 'best', reason: 'TypeScript tells you exactly which properties exist â€” no guessing what\'s on `e.target`.' },
    ...
    commonMistakes: [
      '`onClick={doThing()}` â€” the () calls it immediately during render.',
      'Not calling `e.preventDefault()` on form submit â€” page reloads and state is lost.',
      'Using `onChange` on a checkbox to read `e.target.value` â€” use `e.target.checked` instead.'
    ],
    seniorNotes: '`useCallback` is only beneficial when the handler is passed to a child wrapped in `React.memo`. Profile before adding `useCallback` everywhere â€” the cost of creating a new function on each render is far less than the overhead of unnecessary `useCallback` deps tracking.',
    interviewQuestions: [
  ...
  // Typed onChange â€” React.ChangeEvent<HTMLInputElement>
  function handleEmailChange(e: React.ChangeEvent<HTMLInputElement>) {
    setEmail(e.target.value);
  }

  // Form submit â€” prevent page reload
...

  {
    ...
    eli5: 'Conditional rendering means showing different things depending on a condition â€” like only showing a "Welcome back!" message if you\'re logged in. In JSX you use `?` (if-else) or `&&` (show-if-true).',
    ...
    technicalDeep: 'JSX accepts: strings, numbers, booleans (false, null, undefined render nothing), arrays of elements, or JSX. `false && <X>` renders nothing. `0 && <X>` renders "0" â€” the number zero is truthy-adjacent but renders! Fix: `items.length > 0 && <X>` or `!!items.length && <X>`. Returning `null` from a component renders nothing. Early return pattern: `if (loading) return <Spinner />` is cleaner than deeply nested ternaries. Enum pattern with `Record<Status, JSX.Element>` maps status strings to components without if chains.',
    ...
    commonMistakes: [
      '`{count && <List />}` â€” renders "0" when count is zero.',
      'Deeply nested ternaries â€” extract to a named function or switch statement.',
...
function Page({ status }: { status: Status }) {
...

  {
    ...
    title: 'Lists & Keys',
    eli5: 'To show a list in React, you use `.map()` to turn an array into JSX. Each item needs a `key` â€” a unique ID that React uses to tell items apart when the list changes.',
    ...
    commonMistakes: [
      ...
      'Forgetting the `key` prop entirely â€” React warns but falls back to index behavior.',
      'Not lifting the `key` to the outermost element returned by map â€” key must be on the map\'s direct output.'
    ],
...
// Fragment key â€” group multiple elements per item
function Table({ rows }: { rows: Todo[] }) {
...

  {
    ...
    eli5: 'A controlled input is an input whose value is stored in React state. Every keystroke updates the state, and the state updates the input â€” React is the boss of what the input shows.',
    ...
    explanation: 'Controlled: `<input value={state} onChange={e => setState(e.target.value)} />`. Value comes FROM state; changes go THROUGH setState. Uncontrolled: `<input ref={ref} />` â€” the DOM holds the value, you read it on submit. Controlled is the React standard for forms with validation, formatting, or conditional behavior.',
    technicalDeep: 'Controlled inputs create a React-DOM round trip on each keystroke: event â†’ handler â†’ setState â†’ re-render â†’ DOM update. This is fast but means: (a) you can validate/transform on every keystroke, (b) the input value is always in sync with state. Uncontrolled inputs (`useRef`) bypass this loop â€” better for file inputs (`<input type="file">`) which cannot be controlled, and for performance-sensitive forms. `defaultValue` vs `value`: `value` = controlled (React owns it), `defaultValue` = uncontrolled initial value. Mixing both creates a warning. Form libraries (React Hook Form, Formik) use refs + controlled inputs strategically to minimize re-renders.',
    whatBreaks: '`<input value={state} />` without an `onChange` creates a read-only input (React warns). `<input value={undefined} />` starts as uncontrolled; if state later becomes a string, React switches it to controlled â€” warning about "uncontrolled to controlled" change.',
    efficientWay: {
      ...
      recommendation: 'Learn controlled inputs first â€” you need to understand them to debug React Hook Form. Switch to RHF + Zod for any form beyond a simple login.'
    },
    commonMistakes: [
      '`<input value={value}>` without onChange â€” read-only warning and non-interactive input.',
      'Setting `value={undefined}` initially then a string later â€” uncontrolled/controlled switch warning.',
      'One setState per field when a single object state is cleaner for related fields.'
    ],
    seniorNotes: 'React Hook Form\'s performance advantage (`register` uses refs, re-renders only on submit or validated fields) is significant for large forms. But even with 20 fields, a controlled approach is often fast enough â€” measure before optimizing.',
    interviewQuestions: [
...

  {
    ...
    explanation: 'Prefer composition over complex configuration. Instead of a `<Modal type="confirm" hasFooter showCloseBtn>`, compose: `<Modal><Modal.Header>â€¦</Modal.Header><Modal.Footer>â€¦</Modal.Footer></Modal>`. The `children` prop is the simplest composition primitive. Slot props (`header`, `footer`) offer named injection points.',
    technicalDeep: 'React favors composition over inheritance â€” you cannot meaningfully extend components via prototype chains; you compose them via props/children. Key patterns: (1) Children composition â€” `<Card>{anyContent}</Card>` is maximally flexible. (2) Slot props â€” `<Layout header={<Nav/>} sidebar={<Menu/>}>` lets the parent control each region. (3) Render props â€” `<List renderItem={(item) => <Row data={item} />}` injects rendering logic. (4) Compound components (Context-based) â€” `<Select>` + `<Select.Option>` share state invisibly. (5) HOCs (higher-order components) â€” legacy pattern, mostly replaced by hooks.',
    whatBreaks: 'Trying to extend a component via inheritance (class-based OOP style) â€” React doesn\'t support meaningful component inheritance. Deeply nested composition with many layers of `children` can make the render tree hard to trace in DevTools.',
    efficientWay: {
      ...
      approaches: [
        ...
        { name: 'Extend components by passing many boolean props', verdict: 'weak', reason: 'Boolean prop explosion is a code smell â€” use composition instead.' }
      ],
    ...
    commonMistakes: [
      ...
      'Using React.cloneElement to inject props into children â€” Context or explicit props are cleaner.',
      'Not typing `children: React.ReactNode` â€” causes TypeScript errors when you pass JSX as children.'
    ],
...
// 2. Slot props â€” named injection points
function PageLayout({
...
>
...

  {
    ...
    commonMistakes: [
      ...
      'Passing callbacks 3+ levels deep â€” use Context for callbacks that cross many levels.',
    ...
    codeExamples: [
      {
        ...
        code: `// âŒ Before: two components with their own counters â€” can't sync
...
function Parent() {
...

  {
    ...
    analogy: 'Styling approaches are like painting a house. Global CSS is painting with no tape â€” color can bleed everywhere. CSS Modules is masking tape per room. Inline styles is painting each object individually. Tailwind is color-by-number stickers.',
    ...
    technicalDeep: 'CSS Modules: Vite supports them out-of-the-box. `.module.css` files export an object of local class names. The actual class names are hashed (e.g., `Card_root__x7Qk`) to avoid collisions. Inline styles: `style={{ backgroundColor: "#fff", marginTop: 8 }}` â€” camelCase, no units for 0 values. Only use for truly dynamic values (e.g., a variable color from props). Tailwind: utility-first CSS â€” all styling is done with class names. `cn()` utility (clsx + tailwind-merge) merges conditional class names correctly. CSS-in-JS (Styled Components, Emotion): runtime-generated CSS, more dynamic but slower than static CSS. Falling out of favor vs Tailwind + CSS Modules.',
    whatBreaks: 'Global CSS class name collisions â€” two `.card` classes in different files clash. Inline styles bypass media queries and pseudo-selectors â€” only use for dynamic values. Tailwind\'s `tw-merge` conflict: `px-2 px-4` â€” without merging, both are applied; the last one wins but specificity is confusing.',
    efficientWay: {
    ...
    commonMistakes: [
      'Using inline `style={{ color: \'red\' }}` for static styles â€” hard to override, no dev tools support.',
      'Forgetting to import the `.module.css` file â€” the class works but styles are not applied (silent failure).',
      'Using global CSS class names in React without modules â€” guaranteed collision as the codebase grows.'
    ],
    ...
    codeExamples: [
      {
        ...
        code: `// 1. CSS Modules â€” scope to this file
...
// 4. Inline style â€” only for truly dynamic values from props
<div style={{ width: \`\${progress}%\`, backgroundColor: color }} />`
...

  {
    ...
    eli5: 'React DevTools is a browser extension that lets you peek inside your React components â€” see their state, props, and how fast they render. It\'s like X-ray vision for your app.',
    ...
    technicalDeep: 'Components panel: click any element in the DevTools component tree to see its current state and props. You can even edit state directly for testing. Highlight updates: Settings â†’ "Highlight updates when components render" â€” flashes a colored border on every re-rendering component. Profiler: record a user interaction, then see a flame graph of render times. Each bar is a component; wide bars are expensive. Hooks are listed under "hooks" in the component inspector â€” each hook\'s current value is shown. `displayName`: React DevTools shows the function name; anonymous arrow functions show as "Anonymous" â€” prefer named functions or set `MyComp.displayName = "MyComp"`. `__REACT_DEVTOOLS_GLOBAL_HOOK__` is the extension API â€” you cannot use DevTools to inspect production builds unless you use `react-dom/profiler` and a special build.',
    whatBreaks: 'React DevTools do not work on apps served from `file://` â€” use a local dev server. Production builds minify component names â€” use the profiler build or a source map for production profiling.',
    efficientWay: {
      ...
      approaches: [
        ...
        { name: 'Open the Profiler only when you see a perf problem', verdict: 'best', reason: 'The Profiler is powerful but complex â€” save it for real perf issues.' },
      ...
      recommendation: 'Install DevTools extension immediately. Turn on "Highlight Updates" for your first week â€” you will learn more about React\'s rendering from watching the highlights than from any tutorial.'
    },
    commonMistakes: [
      'Not using DevTools at all â€” debugging React without them is like debugging without a console.',
      'Profiling in development mode â€” development React is 2-3Ã— slower due to extra checks. Profile in production build.',
      'Naming all components as arrow functions assigned to `const` â€” they show as "Anonymous" in DevTools until you add displayName.'
    ],
    seniorNotes: 'The Profiler flame chart shows "Why did this render?" when you enable that setting â€” it lists exactly which state or props change triggered each render. This is the fastest path to finding unnecessary re-renders without guessing.',
    interviewQuestions: [
...
const withAuth = (Component) => {
...

  {
    ...
    eli5: 'A Fragment is an invisible wrapper â€” it lets you return multiple elements without adding an extra `<div>` to the page. A Portal lets you put a component\'s output somewhere else in the HTML (like a modal that goes at the bottom of the page but lives in your component).',
    analogy: 'A Fragment is a clear plastic bag â€” you can bundle items together without a visible box. A Portal is a teleportation device â€” your component lives in the kitchen but its output appears in the living room.',
    explanation: 'Fragments `<></>` group elements without adding DOM nodes. Use keyed fragments `<React.Fragment key={id}>` for lists. Portals render children into a different DOM node: `ReactDOM.createPortal(children, document.getElementById("modal-root"))` â€” used for modals, tooltips, and dropdowns that need to escape overflow:hidden or z-index stacking contexts.',
    technicalDeep: 'Fragment implementation: `React.Fragment` compiles to an internal type that the reconciler handles specially â€” no DOM node is created, just the children are flattened. Short syntax `<>` does not support `key` or other props. `ReactDOM.createPortal(content, targetDOMNode)` renders `content` into `targetDOMNode` but keeps it in the React tree for event bubbling and context purposes. Events on portal content still bubble through the React component tree (not the DOM tree) â€” a click inside a modal portal fires onClick handlers up through its React parents even though the DOM parent is `#modal-root`. Portals require `targetDOMNode` to exist in the HTML â€” typically `<div id="modal-root"></div>` in `index.html`.',
    whatBreaks: 'Using `<>` (short fragment) when you need a `key` â€” `<>` doesn\'t accept props. Using portals for non-overlay content â€” unnecessary complexity. Forgetting to add `#modal-root` to index.html before using a portal to it.',
    efficientWay: {
      ...
      approaches: [
        ...
        { name: 'Build modals as portals', verdict: 'best', reason: 'Portals correctly escape z-index and overflow contexts â€” modals without portals require CSS hacks.' },
      ...
      recommendation: 'Default to `<>` fragments. Add `<div>` wrappers only when you need a DOM element for styling or event handling. Build modals as portals â€” the escaping behavior is essential for correct z-index stacking.'
    },
    commonMistakes: [
      'Using `<>` syntax when the fragment needs a `key` prop â€” use `<React.Fragment key={id}>`.',
      'Building modals without portals â€” they get trapped under overflow:hidden or z-index stacking context.',
      'Querying for the portal target in the component body instead of after mount â€” the element might not exist during SSR.'
    ],
    seniorNotes: 'Portal event bubbling through the React tree (not DOM tree) is both a feature and a gotcha. A click outside a modal portal should close it â€” but `document.addEventListener("click")` fires for ALL clicks including those inside the portal. Solution: `e.stopPropagation()` inside the modal content or an `onMouseDown` on the overlay background.',
    interviewQuestions: [
    ...
    codeExamples: [
      {
        ...
        code: `// Fragment â€” no extra DOM node
...
// Portal â€” render into #modal-root, outside the app div
function Modal({ isOpen, onClose, children }: ModalProps) {
...

  {
    ...
    eli5: 'StrictMode is a special wrapper that makes React complain louder in development about code that could cause bugs. It also runs certain things twice to catch mistakes â€” but only while you\'re developing, not in the real app.',
    ...
    efficientWay: {
      ...
      approaches: [
        ...
        { name: 'Add checks like `if (called) return` to prevent double execution', verdict: 'weak', reason: 'This fights the tool instead of fixing the root cause â€” the impure render.' }
      ],
    ...
    commonMistakes: [
      'Removing `<React.StrictMode>` to silence double renders â€” hiding real bugs.',
      'Being surprised that console.log fires twice in development â€” expected behavior of StrictMode.',
      'Writing fetch calls directly in the render body â€” they fire twice in StrictMode, revealing that they belong in useEffect.'
    ],
    seniorNotes: 'React 18\'s off-screen feature (not yet stable) will unmount/remount components as a performance optimization â€” meaning the double-effect behavior StrictMode emulates in dev will happen in production. Every app built with proper StrictMode compliance is already prepared for this.',
    interviewQuestions: [
    ...
    codeExamples: [
      {
        ...
        code: `// main.tsx â€” Vite default setup
...
useEffect(() => {
  ...
  return () => sub.unsubscribe(); // MUST clean up â€” StrictMode proves it
}, []);
...
// With cleanup: mount â†’ subscribe â†’ unsubscribe â†’ mount â†’ subscribe (1 active = correct)`
  ...
  /* â”€â”€ PHASE 2: React Hooks Deep Dive â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
  {
    ...
    title: 'useEffect â€” Side Effects',
    ...
    technicalDeep: 'Effects run after the browser paints â€” they do not block the UI. `useLayoutEffect` runs synchronously before paint (for DOM measurements). Dependency array: every reactive value (state, props, context, functions defined in render) used inside the effect MUST be in the deps array â€” the `exhaustive-deps` ESLint rule enforces this. Omitting a dep causes stale closure bugs. Object/array deps: `useEffect` compares by reference â€” `{}` !== `{}`. Memoize objects with `useMemo` or `useCallback` before using them as deps. Effects fire after every render where deps changed, not on a schedule. The order: render â†’ browser paint â†’ cleanup previous effect â†’ run new effect.',
    whatBreaks: 'Missing deps cause stale closures. Empty `[]` with a value that changes inside â€” the effect sees the initial value forever. Functions defined in render used as deps â€” they change reference every render, causing infinite loops. Fix: move functions inside the effect or wrap with `useCallback`.',
    efficientWay: {
      ...
      recommendation: 'Enable `eslint-plugin-react-hooks` from day one. Follow the exhaustive-deps rule religiously. For fetching, prefer TanStack Query over raw useEffect â€” it handles caching, deduplication, and stale data automatically.'
    },
    commonMistakes: [
      'Putting an object/array literal in the deps array â€” new reference every render â†’ infinite loop.',
      'Fetching data in useEffect without handling the unmounted component case.',
      'Using `async` directly in useEffect â€” it returns a Promise; use an inner async function instead.'
    ],
    seniorNotes: 'The React team\'s recommendation (React 18+): derive as much as possible in render, not in effects. The new `use()` hook and React Query address the data-fetching-in-effects pattern. If you find yourself writing `useEffect` to sync two pieces of state, something is wrong â€” effects are for synchronizing React with external systems, not for intra-React sync.',
    interviewQuestions: [
  ...
  // Document title side effect â€” runs after every render
  useEffect(() => {
...

  {
    ...
    title: 'useRef â€” Stable References',
    ...
    technicalDeep: 'Refs are instance variables for functional components. The same ref object persists across all renders of the same component instance. `ref.current` is synchronously mutable â€” you can read and write it from anywhere (render, effects, callbacks) without React knowing. DOM refs: `useRef<HTMLInputElement>(null)` â€” typed with the element type, initial `null`. After mount, `ref.current` points to the actual DOM node. Cleanup: when a component unmounts, React sets `ref.current` to `null` for DOM refs. `useRef` vs `useState`: if reading the value in render, use state. If the value should NOT trigger re-render (e.g., a timer ID, a previous value comparison), use ref. `forwardRef` lets parent components pass a ref into a child component â€” needed when a parent needs to focus/scroll a child\'s DOM element.',
    whatBreaks: 'Reading `ref.current` in the render body during the first render â€” DOM refs are null until after mount. Writing to `ref.current` directly to "update" displayed content â€” it won\'t re-render. Using refs instead of state for values that should appear in the UI.',
    efficientWay: {
      title: 'Using useRef',
      approaches: [
    ...
    commonMistakes: [
      'Reading DOM ref.current during render (before mount) â€” it\'s null.',
      'Using ref instead of state for values shown in JSX â€” they won\'t update the display.',
      'Forgetting to type the ref: `useRef<HTMLInputElement>(null)` vs `useRef(null)` â€” the typed version avoids null-check errors.'
    ],
    seniorNotes: '`useRef` is the foundation of `forwardRef` â€” used in every custom input component that needs to be focus-able by parent. React 19 introduces the `ref` prop directly (no `forwardRef` needed) â€” a welcome simplification.',
    interviewQuestions: [
  ...
  useEffect(() => {
    inputRef.current?.focus(); // focus on mount â€” safe here (after paint)
    return () => {
...
// forwardRef â€” parent needs to focus the input
const Input = React.forwardRef<HTMLInputElement, InputProps>((props, ref) => (
...

  {
    ...
    title: 'useMemo â€” Memoized Values',
    ...
    explanation: '`const result = useMemo(() => expensiveCalc(a, b), [a, b])` â€” memoizes the return value of the function and only recomputes when `a` or `b` changes. Use for: expensive calculations, creating stable object/array references for other hooks\' deps.',
    technicalDeep: 'useMemo runs its function synchronously during render, caches the result, and returns the cached value on subsequent renders where deps haven\'t changed. Unlike `useCallback` (memoizes a function), `useMemo` memoizes the function\'s RETURN VALUE. The dependency comparison is Object.is (reference equality for objects). useMemo has a cost: the deps comparison runs every render. Only use when the calculation is actually expensive (>1ms) OR when you need a stable reference for another hook\'s deps array. React may discard the memo cache (e.g., between tab switches) â€” `useMemo` is an optimization hint, not a guarantee.',
    whatBreaks: 'Using `useMemo` for cheap calculations (adds overhead without benefit). Putting object/array literals inside deps of `useMemo` â€” they are new references every render, breaking the memoization. Using `useMemo` to "prevent" renders â€” that\'s `React.memo`\'s job.',
    efficientWay: {
      ...
      approaches: [
        ...
        { name: 'Wrap everything in useMemo proactively', verdict: 'weak', reason: 'Premature optimization â€” adds cognitive load and the deps comparison cost without benefit.' }
      ],
    ...
    commonMistakes: [
      'Using useMemo for simple calculations (`useMemo(() => a + b, [a, b])` â€” just write `const sum = a + b`).',
      'Putting a new object literal in the deps: `useMemo(() => f(), [{ id }])` â€” `{ id }` is new every render.',
      'Expecting useMemo to prevent re-renders of the component â€” it does not, it only caches values.'
    ],
  ...
  // Memoize expensive filter â€” only recomputes when inputs change
  ...
  useEffect(() => {
    chart.update(chartConfig); // chartConfig is stable â€” won't loop
  }, [chartConfig]);
...

  {
    ...
    title: 'useCallback â€” Stable Function References',
    eli5: 'useCallback saves a function between renders. Without it, every render creates a brand new function â€” which can cause child components to think something changed and re-render unnecessarily.',
    ...
    explanation: '`const fn = useCallback(() => doThing(a), [a])` â€” returns the same function reference between renders unless `a` changes. Primarily useful when passing callbacks to memoized children (`React.memo`) or using them as `useEffect` deps.',
    technicalDeep: '`useCallback(fn, deps)` is equivalent to `useMemo(() => fn, deps)` â€” it memoizes the function itself (the value) not its return. A function defined in render has a new reference every render. This matters ONLY when: (1) the function is passed to a `React.memo` child â€” child re-renders because prop reference changed. (2) the function is in a `useEffect` deps array â€” effect re-runs because function reference changed. Without these two scenarios, `useCallback` adds overhead without benefit. The `exhaustive-deps` rule applies: all values from the component scope used in the function must be in the deps array.',
    whatBreaks: 'Wrapping every event handler in `useCallback` by default â€” adds deps array management overhead without benefit for non-memoized children. Omitting deps in the array â€” stale closure.',
    efficientWay: {
      ...
      approaches: [
        ...
        { name: 'Use everywhere "just in case"', verdict: 'weak', reason: 'Adds cognitive and runtime overhead â€” deps arrays can have bugs, adding more is counterproductive.' },
    ...
    commonMistakes: [
      'Wrapping every handler in useCallback â€” almost never beneficial without React.memo.',
      'Omitting state/prop dependencies â€” stale closure inside the callback.',
    ...
    codeExamples: [
      {
        ...
        code: `// Child is expensive to render â€” wrap in memo
...
function Parent({ items }: { items: Item[] }) {
  ...
  }, []); // no deps â€” function doesn't close over any state
...

  {
    ...
    title: 'useContext â€” Consuming Context',
    eli5: 'Context is like a radio station. The provider broadcasts a signal. Any component anywhere in the tree that tunes in (useContext) hears the signal â€” without the signal needing to be passed through every component between them.',
    ...
    technicalDeep: 'Creating context: `const ThemeCtx = createContext<ThemeType>(defaultValue)`. The default is used when a consumer renders outside a Provider. Context value comparison: React uses Object.is â€” if you pass `value={{ theme, toggle }}` (new object every render), every consumer re-renders on every parent render. Fix: memoize the value with `useMemo`. Context does NOT prevent re-renders of consumers â€” every subscriber re-renders on ANY value change, even if they only use a slice. Fix: split context (one per concern) or use a context selector library. `useContext` must be inside a component or hook â€” not at the module level.',
    whatBreaks: 'Passing a new object literal as the context value every render causes all consumers to re-render on every parent render â€” often unnecessary. Single large context (auth + theme + locale) means a theme change re-renders everything reading the context.',
    efficientWay: {
    ...
    commonMistakes: [
      'Passing `value={{ ...stuff }}` inline to Provider â€” new object every render triggers all consumers.',
      'Using context for frequently-updating data (like a live counter) â€” every consumer re-renders on every tick.',
      'Not providing a default value in `createContext` â€” results in TypeScript requiring non-null assertions everywhere.'
    ],
    seniorNotes: 'The "Context vs Zustand" decision: Context is great for low-frequency-update values (auth, theme). Zustand is better for state that many components read AND write frequently â€” it has a built-in subscription model that only re-renders components that subscribed to the specific slice that changed.',
    interviewQuestions: [
...
// 3. Provider â€” memoize value to prevent unnecessary re-renders
...
// 4. Consumer â€” anywhere in the tree
function NavBar() {
...

  {
    ...
    title: 'useReducer â€” Complex State',
    ...
    technicalDeep: 'The reducer pattern is borrowed from Redux (Flux architecture). It enforces: (1) state is immutable â€” always return a new object. (2) transitions are explicit â€” you can read every possible state change in one function. (3) testable â€” pure function, no side effects. TypeScript with discriminated union actions: `type Action = { type: "inc" } | { type: "set"; payload: number }` â€” the switch is exhaustive. Reducer + Context is a lightweight state manager â€” pass `dispatch` via context for shared state. The `useReducer` initializer: third argument is a function `(arg) => initialState` for lazy initialization (same as useState lazy init).',
    whatBreaks: 'Mutating state inside a reducer instead of returning a new object â€” React won\'t detect the change. Dispatching actions inside the reducer instead of from effects/handlers â€” reducers must be pure.',
    efficientWay: {
    ...
    commonMistakes: [
      'Mutating state in the reducer: `state.count++; return state` â€” same reference, React skips re-render.',
      'Dispatching inside effects synchronously without checking if the component is mounted.',
      'Making the reducer impure (async calls, Math.random inside) â€” reducers must be pure.'
    ],
...
function Cart() {
...

  {
    ...
    explanation: 'A custom hook is a function that starts with `use` and can call other hooks. It extracts stateful logic from a component. Name must start with `use` â€” this is how React (and ESLint) enforces the rules of hooks on it. Returns whatever the calling component needs.',
    technicalDeep: 'Custom hooks share stateful logic, not state itself â€” each component calling the hook gets its own isolated state. Rules of hooks apply inside custom hooks: no conditional hook calls. Custom hooks compose: `useAutoSave` might call `useDebounce` and `useMutation`. Testing: custom hooks can be tested with `@testing-library/react`\'s `renderHook`. TypeScript: return type should be explicitly typed for consumer IDE support. Common patterns: `useBoolean` (toggle), `useLocalStorage` (persisted state), `useDebounce`, `useFetch<T>`, `usePrevious`, `useWindowSize`, `useOnClickOutside`, `useMediaQuery`.',
    whatBreaks: 'Not starting the name with `use` â€” the ESLint rules-of-hooks plugin won\'t enforce hook rules inside it. Calling a custom hook conditionally â€” same error as conditional useState.',
    efficientWay: {
      title: 'Building custom hooks',
      approaches: [
        { name: 'Extract a hook the moment you copy-paste stateful logic between two components', verdict: 'best', reason: 'The duplication signal is the right trigger â€” hooks should emerge from real need, not be designed upfront.' },
    ...
    commonMistakes: [
      'Not starting the function name with `use` â€” rules of hooks not enforced.',
      'Returning too many things from a hook â€” group related returns in an object.',
      'Sharing state across callers (trying to use a hook as a singleton) â€” each call gets its own isolated state.'
    ],
    ...
    codeExamples: [
      {
        ...
        code: `// useLocalStorage<T> â€” persisted state
...
// useDebounce<T> â€” debounced value
...
function Search() {
  ...
  // debouncedQuery triggers the API call â€” not every keystroke
}`
      }
    ]
  },

  {
    ...
    explanation: '`const [isPending, startTransition] = useTransition()`. Wrap low-priority updates in `startTransition(() => setHeavyState(val))`. React renders the current state immediately (keeping typing responsive) and processes the heavy update in the background. `useDeferredValue(value)` defers a value for rendering â€” similar but for values you don\'t control.',
    technicalDeep: 'Concurrent React (React 18+) can pause and resume rendering. `startTransition` marks updates as "concurrent" â€” interruptible by urgent updates. While the transition is in progress, `isPending` is true â€” use to show a subtle loading indicator. `useDeferredValue` keeps showing the old value while a new expensive render computes in the background. Both require the expensive rendering to be memoized (wrapped in `useMemo` or `React.memo`) to actually defer work. Without memoization, the deferred value still triggers an immediate re-render.',
    whatBreaks: 'Transitions don\'t help if the expensive component isn\'t memoized â€” React renders it synchronously anyway. Don\'t wrap state updates that must be synchronous (like navigation) in transitions.',
    efficientWay: {
      title: 'Using concurrent features',
      approaches: [
        { name: 'Add useTransition to search/filter that causes typing lag', verdict: 'best', reason: 'The canonical use case â€” keeps the input responsive during expensive list re-renders.' },
        { name: 'Use for every state update', verdict: 'weak', reason: 'Adds complexity for no benefit when renders are fast.' },
        { name: 'Virtualize long lists instead', verdict: 'best', reason: 'react-window/virtual eliminates the rendering cost entirely â€” better than deferring it.' }
      ],
    ...
    commonMistakes: [
      'Expecting useTransition to speed up network requests â€” it only helps with expensive renders.',
      'Not memoizing the expensive component â€” the transition has no effect without React.memo or useMemo.',
      'Using for navigation state â€” navigation should be synchronous and immediate.'
    ],
    seniorNotes: 'The React 18 root API (`createRoot`) is required for concurrent features â€” the legacy `ReactDOM.render` does not support them. Both Vite\'s `react` template and Next.js 13+ use `createRoot` by default.',
    interviewQuestions: [
  ...
  function handleChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setQuery(q); // urgent â€” update input immediately

    startTransition(() => {
      // non-urgent â€” can be deferred if user keeps typing
...
const MemoizedList = React.memo(({ items }: { items: Item[] }) => (
...

  {
    ...
    title: 'React.memo â€” Skipping Re-renders',
    ...
    explanation: '`const MemoComp = React.memo(MyComp)` â€” shallow-compares props before re-rendering. If all props are the same reference (or primitive value), the component skips re-rendering. Must pair with `useCallback`/`useMemo` for function/object props to be stable.',
    technicalDeep: 'React.memo wraps the component in a HOC that does `Object.is` comparison on each prop. Same: skip render. Different: render. For objects/functions, "same" means same reference. Without stable references (from `useCallback`/`useMemo`), every parent render creates new prop references, and `React.memo` never helps. Custom comparison: `React.memo(Comp, (prevProps, nextProps) => prevProps.id === nextProps.id)` â€” return `true` to skip render, `false` to render. The default comparison is shallow (one level deep). `React.memo` should be measured before applied â€” each memo adds comparison overhead every parent render; if the component is cheap, that overhead may be more expensive than the render.',
    whatBreaks: 'Object/array/function props that are recreated every parent render defeat React.memo â€” the comparison always fails. Custom comparison functions that compare incorrectly (returning true when props are different) cause stale renders that show incorrect data.',
    efficientWay: {
      title: 'When to use React.memo',
      approaches: [
        { name: 'Profile first, add memo when you see unnecessary re-renders of expensive components', verdict: 'best', reason: 'Avoid premature optimization â€” most components are fast enough without memo.' },
    ...
    commonMistakes: [
      'Using React.memo without also stabilizing function/object props â€” comparisons always fail.',
      'Memoizing components that re-render because their context changed â€” memo only checks props.',
      'Writing a custom comparison function that omits some props â€” stale data shown in UI.'
    ],
    seniorNotes: 'React Compiler (2024, opt-in in React 19) automatically applies memoization â€” making manual `React.memo`, `useCallback`, and `useMemo` largely unnecessary. When it\'s widely adopted, the current best practice will shift to "let the compiler do it."',
    interviewQuestions: [
    ...
    codeExamples: [
      {
        ...
        code: `// Expensive child â€” wrap in memo
...
function Table({ items }: { items: Item[] }) {
  ...
  // MUST stabilize these â€” otherwise DataRow memo never helps
...

  {
    ...
    explanation: 'Rule 1: Call hooks only at the top level â€” not inside loops, conditions, or nested functions. Rule 2: Call hooks only from React function components or custom hooks â€” not from regular functions, classes, or event handlers. These rules ensure hooks are called in the same order every render.',
    technicalDeep: 'React stores hook state in a linked list indexed by call order. Hook #1 (first useState) is always slot 0, hook #2 is slot 1, etc. If you call a hook conditionally, some renders skip slot N, and every subsequent hook reads from the wrong slot â€” getting another hook\'s state. ESLint plugin `eslint-plugin-react-hooks` with rules `rules-of-hooks` and `exhaustive-deps` enforces both rules. The plugin statically analyzes the code â€” it cannot catch runtime conditions (`if (typeof window !== "undefined")`) inside hooks, so be careful with those. React will throw a helpful error "Rendered more hooks than during previous render" if the rule is violated at runtime.',
    whatBreaks: 'Any conditional hook call (including inside try/catch): React throws "Rendered fewer/more hooks than during previous render" â€” usually immediately visible. Using hooks inside event handlers, `setTimeout` callbacks, or class component methods â€” same error.',
    efficientWay: {
      ...
      approaches: [
        ...
        { name: 'Memorize the rules and rely on code review', verdict: 'weak', reason: 'The ESLint rule catches violations instantly â€” no reason to rely on review.' },
    ...
    commonMistakes: [
      'Calling a hook inside an `if` block to "skip" it when not needed â€” always call it, use the result conditionally.',
      'Early `return` before all hooks are called â€” same violation as a conditional call.',
      'Calling hooks in regular utility functions â€” they must be in components or custom hooks.'
    ],
    seniorNotes: 'The rules of hooks exist because React\'s hook model relies on call order, not a registration system with names. Alternative frameworks (Solid.js, Svelte) use reactive primitives that don\'t have this constraint â€” it\'s a deliberate trade-off React made for simplicity of implementation.',
    interviewQuestions: [
...
function Search({ enabled }: { enabled: boolean }) {
...

  {
    ...
    eli5: 'Once you have a few custom hooks, you can build new hooks on top of them â€” like stacking LEGO bricks. A `useUserProfile` hook might use `useFetch`, `useLocalStorage`, and `useDebounce` all together inside it.',
    analogy: 'Hook composition is like a recipe that calls other recipes. "Make pizza" calls "make dough" and "make sauce" â€” each a recipe itself.',
...
// Layer 3: Feature â€” uses domain hooks
function useProfilePage(userId: number) {
  ...
  /* â”€â”€ PHASE 3: React Architecture & Patterns â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
  {
    ...
    title: 'Context API â€” Patterns & Performance',
    eli5: 'Context lets you share data across many components without passing it through every level. But if you\'re not careful, changing the data makes EVERYTHING using context re-render â€” even parts that don\'t need to change.',
    analogy: 'Context without optimization is a school PA system â€” one announcement wakes up every classroom. Context with split providers is individual classroom phones â€” only relevant classrooms are called.',
    ...
    technicalDeep: 'Context re-render propagation: when Provider value changes, all consumers re-render regardless of which part of the value they use. Patterns to mitigate: (1) Split value and setter into separate contexts â€” `ThemeContext` (value) + `ThemeDispatchContext` (dispatch never changes). Setters from useState are stable between renders; dispatch from useReducer is always stable. (2) `useMemo` on the value object: `const value = useMemo(() => ({ theme, toggle }), [theme])`. (3) Context selector (use-context-selector library): `const theme = useContextSelector(ThemeCtx, ctx => ctx.theme)` â€” only re-renders when the selected slice changes. This pattern is what Zustand implements natively.',
    ...
    efficientWay: {
      ...
      approaches: [
        ...
        { name: 'One big application context', verdict: 'weak', reason: 'Every consumer re-renders on every state change â€” app gets sluggish quickly.' }
      ],
    ...
    commonMistakes: [
      'Single context with many state values â€” a theme change re-renders auth consumers.',
      'Inline object `value={{ user, logout }}` in Provider â€” new object every parent render.',
      'Not defaulting context to null and doing null checks in the custom hook â€” leads to misleading "no value" errors.'
    ],
    seniorNotes: 'The "value and dispatch" pattern (two contexts, one for data, one for the setter) is used in the React docs themselves. It ensures button components that only dispatch events never re-render when data updates â€” a meaningful performance win for complex forms.',
    interviewQuestions: [
...
function ThemeToggle() {
...

  {
    ...
    technicalDeep: 'BrowserRouter uses the History API (pushState) for URL changes without reloads. Route nesting: `<Route path="/user/:id" element={<User />}>` can have child routes rendered via `<Outlet />` in the parent. Protected routes: wrap with a component that checks auth and redirects if unauthenticated. Lazy loading: `<Route lazy={async () => ({ Component: (await import("./Page")).default })} />`. URL parameters: `useParams<{ id: string }>()`. Query strings: `useSearchParams()` â€” returns a URLSearchParams object. Programmatic navigation: `const navigate = useNavigate(); navigate(\'/home\', { replace: true })`. `<NavLink>` auto-adds `active` class when the current URL matches. Data router (v6.4+): `createBrowserRouter` + `loader` functions enable data loading before navigation â€” similar to Next.js but in CSR.',
    whatBreaks: 'Using `<a href="/page">` instead of `<Link to="/page">` triggers a full page reload â€” all React state is lost. Not wrapping the app in `<BrowserRouter>` causes "useNavigate must be used within a Router" error. 404 on direct URL access: the server must serve `index.html` for all routes (SPA fallback), otherwise a page refresh on `/user/5` returns a 404.',
    efficientWay: {
      ...
      approaches: [
        ...
        { name: 'Flat route list with manual layout in every page', verdict: 'ok', reason: 'Works but repeats layout code â€” hard to maintain.' },
        { name: 'Implement routing manually with state', verdict: 'weak', reason: 'The browser back button won\'t work. Never do this.' }
      ],
      recommendation: 'Learn nested routes and `<Outlet>` â€” they model the "layout with changing content" pattern that every real app needs. Add route-level code splitting from day one.'
    },
    commonMistakes: [
      '`<a href>` instead of `<Link to>` â€” full page reload on every navigation.',
      'Not setting up a SPA fallback on the server â€” direct URL access returns 404.',
      'Putting `useNavigate()` call inside non-component functions â€” must be called inside a component.'
    ],
    seniorNotes: 'React Router v6.4\'s data router (`createBrowserRouter`) introduced `loader`, `action`, and `errorElement` â€” enabling data fetching and mutations at the route level with proper pending/error states. This is the evolution toward Next.js-style data handling in CSR apps.',
    interviewQuestions: [
...
// Layout component â€” renders nav + child route content
function AppLayout() {
  return (
    <div>
...
function App() {
...

  {
    ...
    title: 'TanStack Router â€” Type-Safe Routing',
    eli5: 'TanStack Router is like React Router but with a GPS that knows your exact destination type â€” if you try to navigate to a route with the wrong URL params, TypeScript catches it before you even start driving.',
    analogy: 'React Router is a map where you write any destination by hand (and might misspell). TanStack Router is a GPS with verified destinations â€” if the route or params are wrong, it refuses to navigate and tells you at compile time, not at runtime.',
    explanation: 'TanStack Router is a fully type-safe client-side router for React. Unlike React Router, it types your route params, search params, and loaders end-to-end â€” a typo in a route name is a TypeScript error, not a runtime 404. Uses file-based route generation (like Next.js) or code-based config. Growing fast in Vite-first projects.',
    technicalDeep: 'File-based routing: `routes/users/$userId.tsx` creates `/users/:userId` with `userId` typed as string. Code-based: `createRoute({ path: "/users/$userId", validateSearch: z.object({ tab: z.string() }) })`. `useParams()` returns `{ userId: string }` â€” fully inferred. `useSearch()` returns validated search params (Zod schema). Loaders: `loader: ({ params }) => fetchUser(params.userId)` â€” runs before the component renders, result available via `Route.useLoaderData()`. Router devtools: `<TanStackRouterDevtools />`. Route tree generated by CLI: `tsr generate` â€” creates `routeTree.gen.ts` which provides all type information. Link: `<Link to="/users/$userId" params={{ userId: "123" }}>` â€” TypeScript error if the route or params don\'t match. Key difference from React Router v6.4 loaders: TanStack loaders are end-to-end typed including the return data; React Router\'s are not. Search param validation: define a Zod schema per route â€” invalid search params throw a typed error.',
    whatBreaks: 'Forgetting to commit `routeTree.gen.ts` â€” teammates can\'t build. Not running the route generator after adding new route files â€” types are stale. Mixing TanStack Router and React Router in the same app.',
    efficientWay: {
      ...
      approaches: [
        ...
        { name: 'Next.js App Router for SSR/SSG needs', verdict: 'best', reason: 'File-based routing + SSR + streaming â€” the production choice for content or SEO-heavy apps.' }
      ],
    ...
    commonMistakes: [
      'Using string literals in `router.navigate()` instead of typed route objects â€” loses all type safety.',
      'Not running `tsr generate` after adding new route files â€” types become stale.',
      'Choosing TanStack Router then not using its type-safe APIs â€” you gain nothing if you use string paths.'
    ],
    seniorNotes: 'TanStack Router solves the biggest pain point in large SPAs: discovering routing bugs at runtime. When a teammate renames a route, every `<Link>` and `navigate()` call to that route becomes a TypeScript error instantly â€” before anyone runs the app. In large teams with many routes, this is a major DX improvement. The concept (type-safe file-based routing) is also what Next.js App Router provides at the framework level.',
    interviewQuestions: [
...
// In another component â€” TypeScript error if route/params don't match
...

  {
    ...
    efficientWay: {
      ...
      approaches: [
        ...
        { name: 'One top-level error boundary for the whole app', verdict: 'ok', reason: 'Catches everything but shows the same fallback for all errors â€” not user-friendly.' },
    ...
    commonMistakes: [
      'Expecting error boundaries to catch async errors in effects â€” they don\'t; handle those in try/catch.',
      'One giant error boundary at the root â€” shows an empty screen for any minor error.',
      'Not logging errors in `componentDidCatch` â€” you need to know about production crashes.'
    ],
...
function App() {
...

  {
    ...
    eli5: 'Suspense says "if the component below me is waiting for something (loading code or data), show this fallback instead." Code splitting splits your app into chunks that only download when needed â€” making the initial load faster.',
    ...
    explanation: '`React.lazy(() => import("./Component"))` creates a lazy component that loads its code chunk on first render. Wrap with `<Suspense fallback={<Spinner />}>`. Without Suspense, lazy components throw a Promise â€” Suspense catches it and shows the fallback until the Promise resolves.',
    technicalDeep: 'Under the hood: `React.lazy()` returns a component that throws a Promise on first render. Suspense catches the Promise, shows the fallback, and re-renders the lazy component when the Promise resolves (code chunk downloaded). Route-level code splitting is the most impactful: each route\'s component in a separate chunk. In React 18+, Suspense works with data too â€” libraries like React Query and Relay can throw Promises for pending data (experimental). Nested Suspense boundaries: inner boundary catches specific loading, outer boundary is the last resort. `<Suspense>` in server rendering (SSR): Next.js uses Suspense for streaming HTML â€” Server Components stream data, Suspense marks the boundary where HTML streaming happens.',
    whatBreaks: 'Lazy components with named exports: `React.lazy(() => import("./Comp").then(m => ({ default: m.NamedExport })))` â€” lazy only supports default exports directly. Using `React.lazy` outside of `<Suspense>` causes an uncaught Promise.',
    efficientWay: {
      ...
      approaches: [
        ...
        { name: 'Component-level splitting for heavy single components (charts, editors)', verdict: 'best', reason: 'A text editor or chart library can be 500KB â€” only load when that feature is used.' },
      ...
      recommendation: 'Start with route-level lazy loading. Then identify heavy components (>50KB) and lazy-load them. For Next.js, use `next/dynamic` â€” it wraps `React.lazy` with SSR support.'
    },
    commonMistakes: [
      'Using `React.lazy` with named exports â€” only default exports are supported without a wrapper.',
      'Not wrapping lazy components in Suspense â€” causes uncaught error.',
      'Lazy-loading every tiny component â€” creates a waterfall of network requests.'
    ],
    seniorNotes: 'Bundle analysis (`npm run build -- --analyze` with vite-bundle-analyzer plugin) reveals what\'s in each chunk. If a library appears in many chunks, it\'s in the shared chunk â€” good. If a 500KB library appears in only one chunk, lazy-loading that route is worth it.',
    interviewQuestions: [
...
// Each route is a separate chunk â€” only downloaded when the user visits
...
function App() {
...

  {
    ...
    explanation: 'Patterns in order of sophistication: (1) useEffect + fetch (manual, covered already). (2) TanStack Query (`useQuery`, `useMutation`) â€” caching, deduplication, background refetch. (3) SWR (similar to React Query, made by Vercel). (4) Next.js Server Components â€” fetch on the server, stream to client.',
    technicalDeep: 'TanStack Query concepts: `queryKey` â€” unique key per query, used for caching and invalidation. `staleTime` â€” how long data is considered fresh (no refetch). `gcTime` (formerly cacheTime) â€” how long unused data stays in cache. `useQuery` returns `{ data, isLoading, isError, error, refetch }`. `useMutation` returns `{ mutate, isPending, isError }`. Cache invalidation: `queryClient.invalidateQueries({ queryKey: [\'users\'] })` triggers refetch of all matching queries. Optimistic updates: `useMutation` `onMutate` callback. Infinite queries: `useInfiniteQuery` for paginated lists. DevTools: `@tanstack/react-query-devtools` shows all queries, their state, and data.',
    ...
    commonMistakes: [
      'Fetching in the render body â€” infinite loop.',
      'Not providing `QueryClientProvider` at the app root.',
      'Using `isLoading` for mutations â€” use `isPending` instead (renamed in TanStack Query v5).'
    ],
    seniorNotes: 'The separation of "server state" (data from APIs â€” use React Query) and "client state" (UI state like modal open/closed â€” use useState/Zustand) is a key architectural insight. Zustand + React Query handle all state without Redux in 90% of apps.',
    interviewQuestions: [
...
function UserList() {
...

  {
    ...
    title: 'Zustand â€” Client State Management',
    ...
    analogy: 'Zustand is a shared Google Doc. Any component can read it, any component can update it, and every reader sees the update instantly â€” without passing the doc through every door in between.',
    explanation: 'Zustand stores are created with `create()` and accessed via hooks. Components subscribe to specific slices â€” only re-render when that slice changes. No Provider needed. No boilerplate. Use for: UI state shared across many components (sidebar open, filters, cart).',
    technicalDeep: 'Zustand store: `const useStore = create<State>((set, get) => ({ count: 0, inc: () => set(s => ({ count: s.count + 1 })) }))`. `set()` merges shallow â€” no need to spread existing state (unlike useState with objects). `get()` reads current state inside actions. Slice pattern: split large stores into slices combined at `create()` time. Selector for performance: `const count = useStore(s => s.count)` â€” component only re-renders when `s.count` changes. Without selector: `useStore()` â€” re-renders on any store change. Middleware: `devtools()` wraps the store for Redux DevTools support. `persist()` persists to localStorage. `immer()` allows mutating syntax. Zustand vs Context: Context re-renders all consumers on any change; Zustand subscribers only re-render when their selected slice changes.',
    whatBreaks: 'Not using selectors: `const store = useStore()` re-renders the component on every store change. Storing server-fetched data in Zustand â€” use React Query (it handles caching). Circular actions that call each other via `get()`.',
    efficientWay: {
      title: 'Using Zustand',
      approaches: [
    ...
    commonMistakes: [
      'Calling `useStore()` without a selector â€” subscribes to all changes.',
      'Storing API data in Zustand instead of React Query â€” misses caching, background refetch.',
      'Mutating state in actions without `set()` â€” Zustand won\'t detect the change.'
    ],
    seniorNotes: 'Zustand\'s devtools middleware integrates with Redux DevTools extension â€” you get time-travel debugging without Redux\'s overhead. For teams migrating from Redux, this is often the compelling argument.',
    interviewQuestions: [
...
// Selector â€” only re-renders when items.length changes
function CartBadge() {
  const count = useCartStore(s => s.items.length);
  return <span>{count}</span>;
}

// Selector for action only â€” NEVER re-renders (function ref is stable)
function AddToCartButton({ item }: { item: CartItem }) {
...

  {
    ...
    explanation: 'The optimization hierarchy: (1) Find the problem with the Profiler. (2) Move expensive renders down the tree (colocation). (3) Virtualize long lists. (4) Lazy-load heavy components. (5) Memoize with React.memo + useCallback + useMemo. (6) `useTransition` for non-urgent updates. In this order â€” earlier steps give the most value.',
    technicalDeep: 'Colocation: state should live as close to where it\'s used as possible. If a heavy component tree doesn\'t need a piece of state, don\'t put the state above it â€” moving state down avoids re-rendering the expensive subtree. Virtualization: `@tanstack/react-virtual` renders only visible items in a list â€” O(viewport_size) DOM nodes instead of O(list_size). Core Web Vitals: LCP (Largest Contentful Paint) â€” time to render biggest visible element. FID/INP (Interaction to Next Paint) â€” time from user input to response. CLS (Cumulative Layout Shift) â€” layout stability. React DevTools Profiler flame chart shows which components render and how long. Chrome Performance tab shows the actual CPU cost.',
    ...
    efficientWay: {
      ...
      approaches: [
        ...
        { name: 'Virtualize long lists first (>100 items)', verdict: 'best', reason: 'Biggest single performance win in most apps â€” drops DOM nodes from thousands to tens.' },
    ...
    commonMistakes: [
      'Optimizing before profiling â€” fixing the wrong bottleneck.',
      'Rendering 1000+ items in a list without virtualization.',
      'Putting global state above expensive components that don\'t need it â€” re-renders on every state change.'
    ],
...
function VirtualList({ items }: { items: Item[] }) {
  ...
  return (
    <div ref={parentRef} style={{ height: 500, overflow: 'auto' }}>
      {/* Total height â€” keeps scrollbar accurate */}
...
// Renders ~15 DOM nodes regardless of whether items has 100 or 100,000 elements`
...

  {
    ...
    eli5: 'Compound components are a family of components that share hidden state. Think of `<select>` and `<option>` â€” they work together but look separate. Render props let a component share its data with whatever you want to render.',
    ...
    explanation: 'Compound components use Context to share state between parent and children components invisibly. The parent provides state; children consume it. Render props pass the component\'s internals to a function prop: `<List renderItem={(item) => <Row data={item} />}` â€” the consumer controls rendering.',
    technicalDeep: 'Compound pattern: create a context inside the parent; child components use `useContext` to access shared state without explicit prop passing. This is how `<Select>/<Option>`, `<Tabs>/<Tab>`, `<Accordion>/<AccordionItem>` work. Attach children as static properties: `Tabs.Tab = Tab; Tabs.Panel = Panel;` for import ergonomics. Render props: `children` as a function `(data: T) => ReactNode` â€” maximally flexible, caller controls rendering completely. Inversion of control pattern: the library gives you the data, you decide how to render it. Both patterns are used by Radix UI (compound) and Headless UI (compound + render props).',
    whatBreaks: 'Compound children used outside their parent â€” the context won\'t exist and `useContext` returns the default. Check with `if (!ctx) throw new Error(...)` in the custom hook.',
    efficientWay: {
      title: 'Advanced composition',
      approaches: [
        { name: 'Build a compound Tabs component to learn the pattern', verdict: 'best', reason: 'Tabs are simple enough to implement but complex enough to need compound components â€” perfect teaching example.' },
        { name: 'Use Radix UI/Headless UI and read their source', verdict: 'best', reason: 'Production-quality implementations of compound patterns â€” reading them is the best education.' },
    ...
    commonMistakes: [
      ...
      'Overcomplicating simple components with compound patterns â€” use regular props for 1-2 options.'
    ],
    seniorNotes: 'Radix UI\'s entire API is compound components with render slots â€” understanding the pattern lets you use and extend Radix primitives confidently. The `asChild` prop (Radix-specific) is a polymorphic pattern where the component renders as its child instead of a wrapper div.',
    interviewQuestions: [
...
<Tabs defaultTab="home">
...

  {
    ...
    analogy: 'HOC is a superhero cape factory â€” you take any hero (component) and wrap them to add a cape (logic). Render Props is a cafeteria that asks "how do you want your data served?" â€” you hand them a function, they call it with the ingredients.',
    ...
    technicalDeep: 'HOC: `const withAuth = (Wrapped) => (props) => isAuth ? <Wrapped {...props} /> : <Redirect to="/login" />`. Naming convention: `with*`. HOC pitfalls: loses `displayName` in DevTools (fix: `WrappedComponent.displayName = "withAuth(Component)"`), doesn\'t forward refs automatically (fix: `React.forwardRef`), prop collision if HOC and component use the same prop name. Always spread props: `<Wrapped {...props} injectedProp={x} />`. Render Props: `<DataFetcher url="/api/users" render={(data) => <UserList users={data} />} />` â€” or via `children` prop: `<Mouse>{({ x, y }) => <Tracker x={x} y={y} />}</Mouse>`. Modern libraries using HOCs: react-redux `connect()` (replaced by `useSelector`/`useDispatch`), `styled-components` `withTheme` (replaced by `useTheme()`). Custom hook equivalent is always simpler: `const { x, y } = useMouse()`. The evolution: HOC â†’ Render Props â†’ Custom Hook all solve the same problem â€” sharing stateful logic â€” with progressively simpler syntax.',
    whatBreaks: 'Creating HOCs inside a render function â€” a new component is created on every render, breaking React\'s reconciliation and unmounting/remounting on every render. Forgetting to spread original props in HOC â€” parent-passed props are silently lost.',
    efficientWay: {
      ...
      recommendation: 'Understand both patterns well enough to read and refactor legacy code. Write custom hooks for all new logic sharing. The mental model â€” "inject behavior into a component" â€” is still valid; only the syntax changed.'
    },
    commonMistakes: [
      'Creating HOCs inside render functions â€” creates a new component type on every render, breaks state and causes constant unmount/remount.',
      'Not forwarding refs through HOCs â€” breaks parent components that need a DOM ref.',
      'Reaching for Render Props when a custom hook is simpler and available.'
    ],
    seniorNotes: 'The progression HOC â†’ Render Props â†’ Custom Hooks all solve the same problem: sharing stateful logic without inheritance. Understanding this history explains *why* hooks exist and why `useSelector` replaced `connect()`. In senior interviews, explaining this evolution â€” including the specific problems each pattern solved and introduced â€” signals deep React understanding beyond just "hooks are better."',
    interviewQuestions: [
    ...
    codeExamples: [
      {
        ...
        code: `// 1. HOC (2016-2019 era) â€” wraps the component
...
// 2. Render Prop (2017-2019 era) â€” component calls your function
...
// const { width } = useWindowSize();` }
  ...
  /* â”€â”€ PHASE 4: TypeScript with React â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
  {
    ...
    analogy: 'Typed props are a labeled connector panel. Each port has a shape â€” you cannot plug the wrong cable in.',
    explanation: 'Type props with an interface; destructure in the function signature. Use union types for limited options, optional `?` for props with defaults, and `React.ReactNode` for children. Avoid `React.FC` â€” the simpler function form is now preferred.',
    technicalDeep: 'Discriminated union props: `type ButtonProps = { variant: "solid"; color: string } | { variant: "ghost" }` â€” TypeScript narrows inside if/switch. `ComponentProps<"button">` and `ComponentPropsWithoutRef<"input">` extend native HTML props. `HTMLAttributes<HTMLDivElement>` for div wrappers. `as` polymorphic prop: `function Box<T extends React.ElementType = "div">({ as: Comp = "div", ...props }: PolymorphicProps<T>)` â€” advanced pattern used by Radix. `React.ReactNode` = any renderable value. `React.ReactElement` = only JSX elements. `JSX.Element` = alias for `React.ReactElement<any>`.',
    whatBreaks: '`React.FC` implicitly allowed children in older React + TS versions â€” this caused confusing errors. Prefer explicit `children?: React.ReactNode` in the interface.',
    efficientWay: {
      title: 'Typing components',
      approaches: [
        { name: 'Interface first, component body second', verdict: 'best', reason: 'The interface is the public API â€” define it first, the implementation follows.' },
    ...
    commonMistakes: [
      '`React.FC` with generics â€” syntax is awkward; prefer plain function.',
      'Not exporting prop types when they\'re needed by parent components.',
      'Using `any` for event callbacks â€” `(e: React.MouseEvent<HTMLButtonElement>) => void` is specific.'
    ],
    seniorNotes: 'The `ComponentPropsWithRef`/`ComponentPropsWithoutRef` helpers let you build wrapper components that accept all native HTML attributes plus your custom ones â€” essential for design system primitives.',
...
// Usage â€” TypeScript catches wrong props
...
<Button variant="invalid">X</Button> // âŒ TS error: not assignable to 'solid'|'ghost'|'outline'` }]
...

  {
    ...
    technicalDeep: '`useState<T>` â€” TypeScript infers `T` from the initial value. `useState(0)` â†’ `[number, ...]`. `useState(null)` â†’ `[null, ...]` (wrong â€” add generic). `useRef<T>(null)` â€” generic needed for DOM nodes. `useCallback<(id: number) => void>(() => ..., [])` â€” explicit return type catches mistakes. `useReducer` signature: TypeScript infers state and action from the reducer function type â€” usually no explicit annotation needed. Custom hook return types: `as const` on a tuple return: `return [state, setState] as const` so TypeScript knows it\'s `[S, Dispatch<S>]` not `(S | Dispatch<S>)[]`.',
    whatBreaks: '`useState(null)` without a generic makes state permanently `null` type â€” TypeScript errors when you try to set a User. `useRef(null)` without a type creates `RefObject<unknown>` â€” no `.current.value` autocomplete.',
    efficientWay: {
      title: 'Typing hooks',
      approaches: [
        { name: 'Add generic only when inference fails', verdict: 'best', reason: 'TypeScript is smart â€” add generics where inference gives wrong or overly narrow types.' },
    ...
    commonMistakes: ['`useState(null)` â€” state is permanently null type.', '`useRef(null)` without generic â€” no DOM type info.', 'Custom hooks returning tuples without `as const` â€” wrong inferred type.'],
...
// useRef â€” DOM node type
const inputRef = useRef<HTMLInputElement>(null);
// inputRef.current?.focus() â€” safe with optional chaining

// useReducer â€” inferred from reducer fn type
const [state, dispatch] = useReducer(cartReducer, { items: [], total: 0 });

// Custom hook â€” tuple with as const
...

  {
    ...
    efficientWay: {
      ...
      approaches: [
        ...
        { name: 'Cast e.target as HTMLInputElement everywhere', verdict: 'weak', reason: 'Defeats the point â€” use the properly typed event type instead.' },
    ...
    commonMistakes: ['Using `React.MouseEvent` on an input `onChange` â€” should be `React.ChangeEvent<HTMLInputElement>`.', 'Accessing `e.target.value` on a checkbox â€” use `e.target.checked`.', 'Not typing `e.currentTarget` for keyboard events on non-input elements.'],
    seniorNotes: 'The distinction between `target` (element that triggered the event) and `currentTarget` (element the handler is attached to) is especially important for event delegation â€” when one parent handles events for many children.',
...
function SearchBar({ onSearch, onSubmit }: SearchProps) {
...

  {
    ...
    eli5: 'When you use TypeScript with Context, you declare what the context will hold â€” so every component that reads it gets full autocomplete and type checking.',
    ...
    explanation: 'Create context with a specific type: `const AuthCtx = createContext<AuthContextType | null>(null)`. Use a custom hook that throws if used outside the provider â€” this eliminates null checks everywhere.',
    technicalDeep: 'Two approaches: (1) `createContext<T | null>(null)` â€” consumers must handle null. Custom hook throws: `if (!ctx) throw new Error(...)` â€” removes null at every call site. (2) `createContext({} as T)` â€” no null, but consumers outside a Provider get an empty object (silent bugs). Approach 1 is safer. `ReturnType<typeof useAuthProvider>` for inferring context type from the provider function â€” avoids duplicating the type definition. When the context value has actions (functions), type them explicitly: `logout: () => Promise<void>` not `logout: Function`. For large contexts, split into data and actions contexts.',
    whatBreaks: '`createContext(undefined as unknown as AuthCtx)` â€” bypasses the null check with a lie to the compiler. Consumers outside a Provider get undefined at runtime.',
    efficientWay: {
      title: 'Typing context',
      approaches: [
        { name: 'null default + custom hook that throws', verdict: 'best', reason: 'Type-safe and surfaces missing Provider as a clear error.' },
        { name: '`{} as T` default', verdict: 'weak', reason: 'Consumers outside Provider silently get empty object â€” hard to debug.' },
    ...
    commonMistakes: ['`{} as T` default â€” silent bugs when no Provider.', 'Mixing context type and internal implementation â€” export the interface separately.', 'Returning new object from custom hook: `return { user, logout }` â€” creates new ref each call; the context value memoization handles stability.'],
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
...

  {
    ...
    eli5: 'Zod defines WHAT your form data must look like â€” email must be valid, password at least 8 chars. React Hook Form connects that definition to your form inputs, shows errors automatically, and re-renders only when needed.',
    ...
    technicalDeep: 'Zod schema: `const schema = z.object({ email: z.string().email(), password: z.string().min(8) })`. Infer type: `type FormData = z.infer<typeof schema>`. RHF `useForm<FormData>({ resolver: zodResolver(schema) })` â€” validation runs on blur, change, or submit per the `mode` option. `register("fieldName")` returns `{ ref, name, onChange, onBlur }` â€” spreads onto `<input>`. `formState.errors.email?.message` shows the Zod error message. `handleSubmit(onValid, onInvalid)` â€” `onValid` receives typed `FormData`. `Controller` component wraps non-native inputs (custom selects, date pickers). `watch("field")` subscribes to a field value for conditional fields. Performance: RHF renders the form once on mount; each field manages itself via refs â€” no re-renders during typing.',
    whatBreaks: 'Forgetting the `resolver` â€” no schema validation, all data passes. Using controlled inputs with RHF\'s `register` â€” they conflict; use `Controller` for controlled components.',
    efficientWay: {
    ...
    commonMistakes: ['Using `register` on a custom select â€” use `Controller` instead.', 'Not importing `zodResolver` from `@hookform/resolvers/zod`.', '`z.infer` returns the OUTPUT type â€” if you transform fields, the input and output types differ.'],
...
function SignupForm() {
...

  {
    ...
    eli5: 'Advanced TypeScript lets you build components that change their type based on what you give them â€” like a box that knows it holds strings if you put strings in, or numbers if you put numbers in.',
    ...
    technicalDeep: 'Generic component: `function Select<T>({ options, getLabel }: { options: T[]; getLabel: (o: T) => string })` â€” T is inferred from `options`. Polymorphic `as`: `type PolymorphicProps<T extends ElementType> = { as?: T } & ComponentPropsWithoutRef<T>`. `satisfies` operator (TS 4.9): `const routes = { home: "/", about: "/about" } satisfies Record<string, string>` â€” TypeScript checks the type but keeps literal types for autocomplete. Template literal types: `type EventName<T extends string> = \`on\${Capitalize<T>}\`` â€” generates `onClick | onChange | onSubmit`. `NoInfer<T>` (TS 5.4): prevents a type param from being inferred from one argument.',
    whatBreaks: 'Generic components in `.tsx` files: `<T>` is parsed as JSX â€” use `<T,>` or `<T extends unknown>`. Complex discriminated union props with 5+ variants â€” TypeScript error messages become unreadable.',
    efficientWay: {
      ...
      approaches: [
        ...
        { name: 'Build a generic Table component', verdict: 'best', reason: 'Forces you to write a generic that infers column types from data â€” real-world challenge.' },
        { name: 'Avoid advanced patterns until needed', verdict: 'ok', reason: 'Valid â€” don\'t over-engineer. But know the patterns so you can reach for them.' }
      ],
    ...
    commonMistakes: ['`<T>` in .tsx files without trailing comma â€” JSX parse error.', 'Overusing generics where a union type is simpler.', 'Using `any` to escape complex type situations â€” try `unknown` with narrowing first.'],
    seniorNotes: '`satisfies` is underused. `const config = { ... } satisfies AppConfig` checks the shape without widening â€” you keep `config.key` as a literal type, not `string`. Use it for route maps, color palettes, and event name maps.',
    ...
    codeExamples: [{ lang: 'tsx', label: 'Generic Table component', code: `// Generic Table â€” column types inferred from data
interface Column<T> {
  key:    keyof T;
  header: string;
...
// Usage â€” T inferred as User from data
...
<Table data={users} columns={columns} />` }]
  ...
  /* â”€â”€ PHASE 5: React Ecosystem â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
  {
    ...
    explanation: 'Tailwind is a utility-first CSS framework â€” all styling via class names. No custom CSS files for most components. Benefits: no naming things, no unused CSS in production (tree-shaken), consistent spacing/color scale. Pair with `cn()` (clsx + tailwind-merge) for conditional classes.',
    technicalDeep: 'Installation: `npm i -D tailwindcss postcss autoprefixer; npx tailwindcss init -p`. Configure `content` array to point to your source files â€” this is how Tailwind scans for used class names and removes unused CSS. JIT (Just-In-Time) mode: default since Tailwind 3 â€” generates CSS for only used classes on demand. Arbitrary values: `w-[142px]`, `bg-[#61DAFB]` â€” use CSS values not in the scale. `@apply` in CSS files â€” encapsulate utility groups for reuse (use sparingly; defeats the tree-shaking benefit). Dark mode: `darkMode: "class"` â€” add `.dark` class to `<html>`, use `dark:bg-gray-900`. Responsive prefixes: `md:flex lg:grid`. `cn()` implementation: `import { clsx } from "clsx"; import { twMerge } from "tailwind-merge"; export function cn(...args) { return twMerge(clsx(args)); }`. shadcn/ui requires this pattern.',
    whatBreaks: '`content` not configured â€” Tailwind generates 0 CSS. Dynamic class names (`text-${color}-500`) are stripped by JIT â€” use full class names or safelist them. Conflicting utilities (`px-2 px-4`) â€” last class wins without tailwind-merge.',
    efficientWay: {
      title: 'Mastering Tailwind',
      approaches: [
        { name: 'Use Tailwind\'s docs search or a cheat sheet', verdict: 'best', reason: 'The class names follow a consistent pattern â€” you internalize them within a week.' },
        { name: 'Tailwind CSS IntelliSense VS Code extension', verdict: 'best', reason: 'Autocomplete + hover preview for every class â€” not optional.' },
        { name: 'Write all styles in a CSS file using @apply', verdict: 'weak', reason: 'Negates Tailwind\'s main advantages â€” worse tree-shaking, defeats the colocation benefit.' }
      ],
    ...
    commonMistakes: ['Dynamic class names built with string concatenation â€” JIT strips them.', 'Not configuring content paths â€” Tailwind generates nothing.', 'Using Tailwind for complex animations â€” CSS or Framer Motion is better.'],
    seniorNotes: 'Tailwind\'s design token system (the scale for spacing, colors, typography) is also its consistency system â€” using arbitrary values breaks that. Reserve arbitrary values for brand-specific colors that don\'t fit the scale. Extend the config (`theme.extend`) rather than override.',
...
function Badge({ variant, size = 'md', children }: BadgeProps) {
...

  {
    ...
    eli5: 'CSS Modules let you write normal CSS but each class is automatically scoped to one component â€” no more accidental style collisions across the whole app. Sass adds superpowers like variables, nesting, and mixins to regular CSS.',
    analogy: 'Global CSS is a town bulletin board â€” anyone can post and read, names collide. CSS Modules is private mailboxes â€” each component has its own box, and the bundler gives each class a unique hashed name so nothing conflicts.',
    explanation: 'CSS Modules scope class names to the component file â€” `.button` in `Button.module.css` becomes `Button_button__xyz` in the DOM, preventing global collisions. Next.js ships with CSS Modules built in (no setup). Sass (`.scss`) adds variables, nesting, and `@use` imports â€” many enterprise codebases use Sass + CSS Modules together.',
    technicalDeep: 'CSS Modules: filename must end in `.module.css`. Import: `import styles from "./Button.module.css"`. Apply: `<button className={styles.button}>`. Multiple classes: use `clsx` library â€” `clsx(styles.button, isPrimary && styles.primary)`. Dynamic key: `styles[variantName]`. Global override: `:global(.some-class) { }` for third-party classes. Compose from another module: `composes: baseBtn from "./Base.module.css"`. TypeScript: `*.module.css` declaration file or `typescript-plugin-css-modules` for typed imports (`styles.button` autocompletes). Sass: `npm i sass` â€” rename to `.module.scss`, both features work together. Nesting: `.card { &:hover { box-shadow: ...; } }`. Variables: `$primary: #a78bfa`. Mixins: `@mixin flex-center { display: flex; align-items: center; justify-content: center; } .container { @include flex-center; }`. Use `@use` (modern) not `@import` (deprecated). Naming convention: camelCase in CSS Modules â€” `.primaryButton` not `.primary-button` (bracket notation works but is ugly).',
    whatBreaks: 'Forgetting `.module.css` extension â€” classes become global and collide. Dynamic class names with typos: `styles[dynamicClassName]` silently returns `undefined`, no error. Vendor-prefixed properties still need PostCSS autoprefixer even in CSS Modules.',
    efficientWay: {
      ...
      approaches: [
        ...
        { name: 'CSS Modules for enterprise/library work', verdict: 'best', reason: 'Full CSS power, scoped, zero runtime, supported everywhere â€” common in older enterprise codebases.' },
        { name: 'styled-components / emotion', verdict: 'ok', reason: 'Runtime CSS-in-JS â€” fine for teams that prefer it, but adds runtime cost and complexity.' }
      ],
      recommendation: 'Learn Tailwind for new projects. Learn CSS Modules to read existing enterprise code â€” Next.js docs use them, many company codebases do. You will encounter both in job environments.'
    },
    commonMistakes: [
      'Using hyphenated class names without camelCase â€” `styles["primary-button"]` works but `styles.primaryButton` is cleaner and autocompletes.',
      'Writing global styles inside a module file without `:global()` wrapper â€” accidentally scopes things you wanted global.',
      'Nesting too deep in Sass â€” 3 levels max, or specificity becomes unmanageable.'
    ],
    seniorNotes: 'Many enterprise React/Next.js codebases (Vercel, Shopify, GitHub) still use CSS Modules as their primary styling â€” Tailwind was not the default 4-5 years ago when those codebases started. Reading `.module.css` files is a day-one required skill for any large codebase. When you join a company using CSS Modules, the mental model transfers instantly from any CSS you already know.',
    interviewQuestions: [
...
export function Button({
  ...
  return (
    <button
      className={clsx(
        styles.button,
        styles[variant],              // dynamic lookup â€” 'primary' | 'danger'
...

  {
    ...
    title: 'shadcn/ui â€” Component Library',
    ...
    explanation: 'shadcn/ui is not a npm package â€” components are added via CLI (`npx shadcn-ui@latest add button`) into your `src/components/ui/` folder. You own and edit the code. Built on Radix UI primitives (accessible, headless) + Tailwind CSS. Requires the `cn()` utility.',
    technicalDeep: 'Architecture: Radix UI provides behavior + accessibility (focus management, keyboard nav, ARIA). Tailwind provides styling. You provide customization. Setup: `npx shadcn-ui@latest init` â€” creates `components.json`, adds `cn()` to `lib/utils.ts`, configures Tailwind CSS variables. Add components: `npx shadcn@latest add dialog card button` â€” each component is added as a file you own. `cva` (class-variance-authority) creates variant classes: `const buttonVariants = cva("base classes", { variants: { variant: { solid: "...", ghost: "..." } } })`. `Slot` from Radix (`asChild` prop): renders the component as its child element â€” polymorphic without TypeScript generics complexity.',
    whatBreaks: 'Not running `shadcn init` first â€” components won\'t have the right CSS variable setup. Editing shadcn components in node_modules instead of your `ui/` folder â€” your edits are lost on `npm install`.',
    efficientWay: {
      title: 'Using shadcn/ui',
      approaches: [
        { name: 'Start every new project with shadcn/ui', verdict: 'best', reason: 'Zero design decisions for common components â€” focus on the product, not the button.' },
      ...
      recommendation: 'Run `npx shadcn@latest init` at project start. Add components as you need them. Customize via the CSS variables in `globals.css` for theming â€” don\'t edit each component.'
    },
    commonMistakes: ['Editing components in a package instead of the owned file.', 'Not setting up CSS variables â€” components display with wrong colors.', 'Trying to npm install shadcn/ui â€” it\'s not a package, it\'s a CLI.'],
    seniorNotes: 'The "copy into your project" model solves the most common pain with component libraries: you can\'t change internal behavior or styles without forking. shadcn/ui makes that the default â€” you own the code, upgrade is opt-in per component.',
...
# Components appear in src/components/ui/ â€” you own them
...
# src/components/ui/dialog.tsx` }]
...

  {
    ...
    eli5: 'Tests check that your components work as expected. React Testing Library tests components from the user\'s perspective â€” click this button, see if this text appears. Vitest is the fast test runner that runs the tests.',
    ...
    technicalDeep: 'Setup: `npm i -D vitest @testing-library/react @testing-library/user-event @testing-library/jest-dom jsdom`. `vitest.config.ts`: `environment: "jsdom"`. `setupFiles`: import `@testing-library/jest-dom` for matchers like `toBeInTheDocument`. Query priority: `getByRole` > `getByLabelText` > `getByPlaceholderText` > `getByText` > `getByTestId`. Role-based queries (`getByRole("button", { name: /submit/i })`) are most resilient to refactoring. `userEvent.setup()` simulates real browser events with timing. `screen.findBy*` returns a Promise â€” use `await` for async rendering. `renderHook` from RTL tests custom hooks. `vi.mock("module")` mocks imports. MSW (Mock Service Worker) intercepts fetch calls â€” better than mocking `fetch` directly.',
    whatBreaks: 'Testing implementation details (`getByTestId`, querying CSS classes) makes tests fragile â€” they break on refactoring even when behavior is correct. `act()` warnings mean your test has unhandled async updates â€” use `await userEvent` and `await findBy*`.',
    efficientWay: {
      ...
      approaches: [
        ...
        { name: 'Use MSW for API mocking', verdict: 'best', reason: 'Mock at the network layer â€” components don\'t know they\'re in a test.' }
      ],
    ...
    commonMistakes: ['`getByTestId` overuse â€” brittle tests that break on restructure.', '`act()` warnings ignored â€” they indicate async state updates not handled in the test.', 'Mocking the entire module instead of just the network layer.'],
    seniorNotes: 'The testing trophy (Kent C. Dodds): most tests should be integration tests (component trees with mock network), fewer unit tests (pure functions, custom hooks), and a handful of e2e tests (Playwright). Not the testing pyramid from backend â€” React integration tests are cheap.',
...
describe('LoginForm', () => {
...

  {
    ...
    eli5: 'Unit tests check one function. Integration tests check one component. E2E (end-to-end) tests check that a real user can complete a real task â€” like registering, logging in, and checking out â€” in a real browser.',
    analogy: 'Unit tests are checking each car part. Integration tests are checking the engine assembly. E2E tests are a test driver completing the full driving exam â€” get in, start car, drive the route, park, pass.',
    explanation: 'Playwright automates a real browser (Chromium, Firefox, WebKit). You write tests that navigate pages, fill forms, click buttons, and assert what the user sees. E2E tests are slower than unit tests but give the highest confidence â€” they catch bugs that only appear in the full running app.',
    technicalDeep: 'Setup: `npm init playwright@latest` â€” creates `playwright.config.ts` and `tests/` folder. Core API: `test("name", async ({ page }) => { })`. `page.goto("/")` navigates. `page.getByRole("button", { name: /submit/i })` finds by ARIA role (same priority as RTL). `page.getByLabel("Email")` for form inputs. `expect(page).toHaveURL("/dashboard")`. `expect(page.getByText("Welcome")).toBeVisible()`. `page.fill()` or `.fill()` on a locator. Authentication: create a `storageState` fixture that logs in once and reuses auth cookies â€” avoids logging in for every test (10x speed improvement). Visual regression: `await expect(page).toHaveScreenshot()` â€” pixel diff vs baseline. Trace viewer: `npx playwright show-trace trace.zip` â€” shows every action + screenshot + network request. CI: headless by default, `npx playwright test --reporter=html`. Page Object Model: class encapsulating page interactions â€” `class LoginPage { async login(email, pass) { ... } }` â€” makes tests readable and resilient to UI changes.',
    whatBreaks: 'Using `setTimeout()` waits â€” root cause of flaky tests. Playwright auto-waits; never sleep. Hardcoding `localhost:3000` â€” use `baseURL` in `playwright.config.ts`. Running E2E tests against production DB accidentally.',
    efficientWay: {
      title: 'E2E testing strategy',
      approaches: [
        { name: 'E2E for critical user flows only', verdict: 'best', reason: 'Login, signup, checkout â€” 5-10 flows. E2E is slow; don\'t replace RTL integration tests.' },
    ...
    commonMistakes: [
      'Using `setTimeout` instead of Playwright\'s auto-waiting â€” root cause of flaky tests.',
      'Not using `storageState` for auth â€” logging in for every test slows the suite 10x.',
      'Writing E2E for things RTL already covers (component rendering, isolated form validation).'
    ],
    seniorNotes: 'Playwright\'s auto-waiting is its killer feature â€” every `click()`, `fill()`, `getByRole()` automatically waits for the element to be visible, enabled, and stable. This eliminates the #1 cause of flaky tests in Selenium/Cypress. The trace viewer makes debugging CI failures trivial â€” you see a video + screenshot + network log of exactly what happened.',
    interviewQuestions: [
...
export default defineConfig({
...

  {
    ...
    eli5: 'Framer Motion makes animating React components easy. Instead of writing complex CSS animations, you describe: where the element starts, where it ends, and how it gets there â€” Framer handles the rest.',
    ...
    technicalDeep: '`motion.div` wraps any HTML element. Props: `initial={{ opacity: 0, y: -20 }}` â€” start state. `animate={{ opacity: 1, y: 0 }}` â€” end state. `exit={{ opacity: 0 }}` â€” unmount state (requires `<AnimatePresence>`). `transition={{ duration: 0.3, ease: "easeOut" }}`. `whileHover={{ scale: 1.05 }}` â€” gesture states. `variants` prop for complex animations: define named states, animate by name. `staggerChildren` in the parent variant staggers child animations. Layout animations: `layout` prop â€” animates a component\'s position/size change automatically when surrounding DOM changes. `LayoutGroup` for animating layout changes across multiple components. `useMotionValue` + `useTransform` for scroll-linked animations.',
    whatBreaks: 'Missing `<AnimatePresence>` around conditionally rendered `motion.*` components â€” exit animations don\'t play. Animating CSS properties that cause layout reflow (width, height, top, left) instead of `transform` equivalents â€” janky 60fps performance.',
    efficientWay: {
      ...
      approaches: [
        ...
        { name: 'CSS transitions for simple hover/state changes', verdict: 'best', reason: 'CSS transitions are lighter for simple cases â€” use Framer for orchestration and complex sequences.' },
    ...
    commonMistakes: ['No AnimatePresence for exit animations.', 'Animating properties that cause reflow (width, margin) â€” use transform instead.', 'Importing the whole library â€” use `framer-motion/dist/framer-motion` selective import or the slim build.'],
...
function AnimatedList({ items }: { items: Item[] }) {
...

  {
    ...
    technicalDeep: 'ESLint config for React: `eslint-plugin-react`, `eslint-plugin-react-hooks` (critical â€” enforces hooks rules), `eslint-plugin-jsx-a11y` (accessibility). `.eslintrc` with `extends: ["plugin:react-hooks/recommended"]`. Prettier: runs after ESLint (`eslint-plugin-prettier` or run separately). lint-staged: runs linters only on staged files â€” fast pre-commit. Husky: `npx husky init` â€” creates `.husky/pre-commit` hook. Storybook: `npx storybook@latest init` â€” each story is one component state. Stories drive component-first development and serve as living documentation. Playwright: `npm init playwright@latest` â€” real browser E2E tests. `page.getByRole()` mirrors RTL\'s accessibility-first approach. `page.goto()`, `page.click()`, `expect(page).toHaveURL()`.',
    whatBreaks: 'Not running `eslint-plugin-react-hooks` â€” hooks rules violations make it to production. Prettier + ESLint conflicts â€” use `eslint-config-prettier` to disable conflicting ESLint formatting rules.',
    efficientWay: {
    ...
    commonMistakes: ['Missing `eslint-plugin-react-hooks` â€” the most impactful plugin.', 'ESLint and Prettier fighting each other without `eslint-config-prettier`.', 'Committing without running tests â€” add `npm test -- --run` to the Husky pre-commit hook.'],
    seniorNotes: 'The Husky + lint-staged setup is non-negotiable in team projects â€” it prevents code style debates and catches hooks violations before they land in CI. The 30 seconds of pre-commit time is worth it.',
...
// npx lint-staged` }]
  ...
  /* â”€â”€ PHASE 6: Next.js Foundations â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
  {
    ...
    eli5: 'Next.js is React with superpowers. It adds: pages that load from the server (faster first paint, better SEO), automatic file-based routing, image/font optimization, and built-in API routes â€” all with one `npm run dev`.',
    analogy: 'React is an engine. Next.js is the full car â€” engine + chassis + transmission + navigation + air conditioning. You could assemble all those from parts, but why when the car is ready to drive?',
    ...
    technicalDeep: 'Next.js rendering modes: (1) Static Generation (SSG) â€” HTML built at build time. (2) SSR â€” HTML rendered on each request. (3) ISR â€” static HTML regenerated after N seconds on request. (4) Client-side rendering (CSR) â€” React running in browser only. (5) React Server Components (RSC) â€” component runs on server, streams HTML to client, zero client JS. App Router (`app/`) vs Pages Router (`pages/`): App Router is the new default â€” uses RSC, layouts, streaming. Pages Router is stable legacy â€” uses `getServerSideProps`, `getStaticProps`. File conventions: `page.tsx` (route), `layout.tsx` (shared wrapper), `loading.tsx` (Suspense fallback), `error.tsx` (error boundary), `not-found.tsx`. TypeScript: Next.js generates types for routes and params automatically â€” use `PageProps` and `LayoutProps` from `next`.',
    whatBreaks: 'Mixing App and Pages routers in the same project â€” possible but confusing. Using `useState` in a Server Component â€” Server Components have no client-side state. Third-party libraries that access `window`/`localStorage` in Server Component context â€” they crash on the server.',
    efficientWay: {
      ...
      recommendation: 'Start with App Router for new projects â€” it\'s the future of Next.js. Learn Pages Router conceptually for job interviews, since many existing projects use it.'
    },
    commonMistakes: ['Using `useState` or `useEffect` in a Server Component â€” add `"use client"` at the top.', 'Not understanding which components run on server vs client.', 'Fetching data in client components when Server Components could do it more efficiently.'],
    seniorNotes: 'Next.js\'s default behavior changed significantly with the App Router: components are Server Components by default. This inverts the "everything is client-side" React mental model. The key insight: Server Components run once at request time on the server, no JS sent to client â€” perfect for database queries and large dependencies.',
...
#         page.tsx  â† /blog/:slug route` }]
...

  {
    ...
    eli5: 'REST APIs are like a fixed restaurant menu â€” you order dish A and get exactly what\'s listed. GraphQL is like ordering custom â€” you describe exactly what fields you want, and get precisely that, nothing more, nothing less.',
    ...
    explanation: 'GraphQL is a query language for APIs. Instead of multiple REST endpoints, there\'s one endpoint (`/graphql`) where the client specifies exactly which fields it needs. This eliminates over-fetching (getting unused fields) and under-fetching (needing multiple requests). Many large companies â€” GitHub, Twitter/X, Shopify, Meta â€” use GraphQL.',
    technicalDeep: '`gql` tagged template: `const GET_USER = gql\\`query GetUser($id: ID!) { user(id: $id) { name email } }\\``. Apollo Client setup: `new ApolloClient({ uri: "/graphql", cache: new InMemoryCache() })`, wrap app in `<ApolloProvider client={client}>`. `useQuery(GET_USER, { variables: { id } })` returns `{ data, loading, error }`. `useMutation(CREATE_POST)` returns `[createPost, { loading, error }]`. Cache normalization: Apollo stores responses by `__typename + id` â€” mutations that return the right fields auto-update every `useQuery` for that entity. `refetchQueries` for explicit cache invalidation. Fragments: `fragment UserFields on User { id name email }` for reusable field sets. Codegen: `graphql-codegen` generates TypeScript types from the schema â€” never manually type API responses. Core concepts: Query (read), Mutation (write), Subscription (real-time over WebSocket). N+1 problem: one GraphQL query can trigger hundreds of DB queries without DataLoader on the server.',
    whatBreaks: 'Not handling `loading` state â€” briefly shows undefined data. Ignoring N+1 on the server â€” GraphQL queries can fan out to hundreds of DB calls. Not using codegen â€” manually typed responses drift from the schema.',
    efficientWay: {
      title: 'REST vs GraphQL â€” when to choose what',
      approaches: [
        { name: 'REST + TanStack Query for most projects', verdict: 'best', reason: 'Simpler, widely understood, excellent HTTP caching, easier to debug â€” covers 90% of use cases.' },
        ...
        { name: 'Learn GraphQL to READ existing APIs', verdict: 'best', reason: 'GitHub, Shopify, and Contentful all have GraphQL APIs â€” you will encounter it in the wild.' }
      ],
    ...
    commonMistakes: [
      'Not using codegen â€” manually typing API responses is slow and drifts from the schema.',
      'Using Apollo where TanStack Query + REST would work fine â€” Apollo adds significant bundle and complexity.',
      'Ignoring the N+1 problem on the server â€” one GraphQL query can trigger hundreds of DB queries without DataLoader.'
    ],
    seniorNotes: 'The real value of GraphQL isn\'t the query syntax â€” it\'s the Apollo Client cache that normalizes entities globally (update a user in one mutation and every `useQuery` that includes that user auto-updates without refetching). At scale with multiple client teams consuming the same API, this is transformative. But it adds engineering overhead â€” evaluate the tradeoff honestly before committing.',
    interviewQuestions: [
...
function UserProfile({ userId }: { userId: string }) {
...

  {
    ...
    explanation: 'App Router maps `app/` folder structure to URLs. `page.tsx` renders a route. `layout.tsx` wraps a route and its children â€” persists across navigation. `loading.tsx` is the Suspense fallback. `error.tsx` is the error boundary. All are Server Components by default.',
    technicalDeep: 'Route segments: `app/dashboard/settings/page.tsx` â†’ `/dashboard/settings`. Dynamic segments: `app/blog/[slug]/page.tsx` â†’ `/blog/any-slug`. Catch-all: `[...slug]`. Optional catch-all: `[[...slug]]`. Route groups: `(marketing)/about/page.tsx` â€” parentheses exclude from URL, used to share layouts without URL nesting. Parallel routes: `@modal/page.tsx` â€” render multiple pages in the same layout simultaneously. Intercepting routes: `(.)photo/[id]/page.tsx` â€” intercept route and show modal instead of full page. `generateStaticParams()` in dynamic routes for static generation. `generateMetadata()` for dynamic SEO metadata. Layout persistence: layouts re-render on navigation ONLY if their segment is different â€” the root layout NEVER re-renders between navigations (preserving scroll position, form state, etc.).',
    whatBreaks: 'Trying to use `useState` directly in a layout or page file without `"use client"`. Layouts re-render when params change â€” if you need the layout to NOT re-render on param change, use the group pattern.',
    efficientWay: {
    ...
    commonMistakes: ['No `"use client"` on interactive components â€” Server Component has no state/effects.', 'Putting the wrong content in layout vs page â€” layout persists, page changes.', 'Not using route groups for shared layouts without URL coupling.'],
    seniorNotes: 'Layout persistence is Next.js\'s answer to the "navigation loses scroll position" problem. Because layouts never unmount between same-segment navigations, you can keep a sidebar scroll position, open accordion state, or playing audio across page navigations â€” React state in the layout survives.',
    ...
    codeExamples: [{ lang: 'tsx', label: 'App Router file structure and layout nesting', code: `// app/layout.tsx â€” root layout (NEVER unmounts)
...
// app/(dashboard)/layout.tsx â€” dashboard layout (persists within /dashboard/*)
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex">
      <Sidebar />          {/* persistent â€” keeps scroll, state */}
      <main>{children}</main>
    </div>
  );
}

// app/(dashboard)/users/[id]/page.tsx â€” dynamic route
export default async function UserPage({ params }: { params: { id: string } }) {
...

  {
    ...
    analogy: 'Parallel routes are like a split-screen TV â€” two channels running simultaneously, each controllable. Intercepting routes are like a photo lightbox â€” click a photo and a modal appears, but if someone pastes you the direct link they see the full photo page.',
    explanation: 'Parallel Routes (`@slot` folders) render multiple pages simultaneously in one layout â€” used for dashboards with independent panels. Intercepting Routes (`(.)folder`) show a different UI when navigating from the same layout, but the full page when navigating directly â€” the pattern behind Instagram/Pinterest-style modals.',
    technicalDeep: 'Parallel Routes: create `@`-prefixed folders â€” `app/dashboard/@analytics/page.tsx` and `app/dashboard/@team/page.tsx`. The dashboard layout receives both as props: `export default function Layout({ analytics, team, children }: Props)`. Each slot has independent loading/error states and its own navigation. `default.tsx` is required as a fallback when a slot has no active match (else hard navigation breaks). Soft (client-side) navigation shows both slots; hard navigation (refresh) uses `default.tsx`. Intercepting Routes: `(.)` intercepts same level, `(..)` one level up, `(...)` root level. Example: `app/feed/@modal/(.)photo/[id]/page.tsx` intercepts `/photo/[id]` when navigating from `/feed` â€” shows as modal. Navigating directly to `/photo/[id]` shows the real page. The full Instagram modal pattern: (1) Parallel route `@modal` in the feed layout. (2) Intercepting route `(.)photo/[id]` inside `@modal`. (3) Modal component with `router.back()` to close. (4) `default.tsx` returning `null` so `@modal` is invisible by default.',
    whatBreaks: 'Missing `default.tsx` in parallel route slots â€” page refresh returns 404 for the slot. Forgetting that intercepting routes only work with soft navigation â€” direct URL always shows the unintercepted page. Using parallel routes when a simple flex layout would do.',
    efficientWay: {
      ...
      approaches: [
        ...
        { name: 'Intercepting Routes for the "modal with shareable URL" pattern', verdict: 'best', reason: 'The canonical Next.js solution â€” open in modal from list, full page from direct link.' },
        { name: 'Avoid for simple use cases', verdict: 'best', reason: 'Both patterns add file structure complexity â€” only use them when the navigation behavior requires it.' }
      ],
      recommendation: 'Learn by building the photo gallery modal example â€” it\'s the clearest demo of both features together. This is a common technical interview topic for mid-senior Next.js roles.'
    },
    commonMistakes: [
      'Missing `default.tsx` in `@modal` slot â€” page refresh crashes the layout with 404.',
...
export default function PhotoModal({ params }: { params: { id: string } }) {
...

  {
    ...
    eli5: 'Server Components run on the server and send HTML â€” they can talk to databases and keep secrets. Client Components run in the browser â€” they can react to clicks and remember state. Most components should be Server Components.',
    analogy: 'Server Components are chefs in the kitchen â€” they see all the raw ingredients (database, secrets) and prepare the dish. Client Components are waiters â€” they interact with the customer and bring the food, but they never see the kitchen.',
    ...
    technicalDeep: 'Server Components: run at request time on the server. Zero client JavaScript. Can `await` data, use Node APIs, access environment secrets. Serialized as HTML + JSON flight data. Cannot use: useState, useEffect, browser APIs, event handlers. Client Components: `"use client"` at top of file â€” this marks a "client boundary." All imports of a Client Component are also client-bundled. Server Components CAN be children/props of Client Components (they\'re rendered by the server and passed as already-rendered HTML). This is the "server component as children" pattern â€” critical for keeping the server/client boundary in the right place. Data flow: Server Component fetches data â†’ passes as props to Client Component (which is serialized as JSON â€” no class instances, functions, or Dates without serialization).',
    whatBreaks: 'Passing non-serializable values (functions, class instances, Dates) from Server to Client Components as props â€” they cannot cross the serialization boundary. Using browser APIs (localStorage, window) in a Server Component â€” they don\'t exist on the server.',
    efficientWay: {
      title: 'Deciding server vs client',
      approaches: [
        { name: 'Start everything as Server Component, add "use client" only when needed', verdict: 'best', reason: 'Push client boundary as far down the tree as possible â€” minimize client JS.' },
        { name: 'Add "use client" to all components for simplicity', verdict: 'weak', reason: 'Defeats the purpose of Server Components â€” you\'re back to a plain React SPA.' },
    ...
    commonMistakes: ['`"use client"` on a parent when only one child needs interactivity â€” add it to the child instead.', 'Importing a client-only library (e.g., chart.js) in a Server Component â€” crashes.', 'Passing a Date object as prop to a Client Component â€” serialize to ISO string first.'],
    seniorNotes: 'The "donut" pattern: Server Component (outer) â†’ Client Component shell (with "use client") â†’ Server Component children (passed via props). This lets you have a client-side interactive wrapper with server-rendered content inside â€” common for modals, sidebars, and carousels.',
    ...
    codeExamples: [{ lang: 'tsx', label: 'Server â†’ Client boundary', code: `// app/users/page.tsx â€” Server Component (no "use client")
...
// components/UserTable.tsx â€” Client Component
'use client';
function UserTable({ users }: { users: User[] }) {
...

  {
    ...
    analogy: 'Metadata is the label on a package. Before anyone opens it (visits the page), they read the label â€” title, description, preview image. Next.js writes the label automatically.',
    explanation: 'Export a `metadata` object (static) or `generateMetadata` function (dynamic) from `page.tsx` or `layout.tsx`. Next.js injects the correct `<title>`, `<meta>`, and OG tags. Layouts\' metadata merges with pages\' metadata â€” pages override.',
    technicalDeep: 'Static: `export const metadata: Metadata = { title: "Home", description: "..." }`. Dynamic: `export async function generateMetadata({ params }: { params: { id: string } }): Promise<Metadata> { const post = await getPost(params.id); return { title: post.title, openGraph: { images: [post.image] } } }`. Template: `title: { template: "%s | My Site", default: "My Site" }` â€” child pages fill `%s`. `Metadata` type from `next`: covers title, description, openGraph, twitter, icons, robots, canonical, alternates. Robots: `export const metadata = { robots: { index: false, follow: false } }` for private/draft pages. `viewport` metadata moved to a separate `export const viewport: Viewport = { themeColor: "#000" }` in Next.js 14+.',
    whatBreaks: 'Using `<title>` directly in JSX inside a layout â€” Next.js metadata system handles this. Duplicate `<title>` tags break SEO.',
    efficientWay: {
      ...
      approaches: [
        ...
        { name: 'react-helmet (React SPA approach)', verdict: 'weak', reason: 'Helmet manipulates the DOM â€” Server Components render metadata in the HTML, not client-side.' },
    ...
    commonMistakes: ['Missing `og:image` â€” social sharing shows no preview.', 'Not using the title template â€” every page shows the bare page title with no site name.', 'Setting robots noindex on production pages accidentally.'],
    seniorNotes: 'OG image generation: Next.js has built-in `ImageResponse` from `next/og` â€” generates OG images on-demand with JSX. No design tool needed: `export async function GET(req) { return new ImageResponse(<div style={{...}}>Post Title</div>) }`.',
    ...
    codeExamples: [{ lang: 'tsx', label: 'Static and dynamic metadata', code: `// app/layout.tsx â€” base metadata for the whole site
...
// app/blog/[slug]/page.tsx â€” dynamic metadata per post
export async function generateMetadata(
...

  {
    ...
    analogy: 'Next.js Link is a smart highway â€” it pre-builds the exit ramp before you arrive so you drive through instantly.',
    ...
    technicalDeep: 'Link prefetching: in production, `<Link>` components in the viewport are prefetched automatically â€” the target page\'s Server Component data is fetched and cached before the user clicks. `prefetch={false}` to disable. Soft navigation: Next.js uses the browser history API â€” no full page reload, React state in persistent layouts is preserved. `useRouter` from `next/navigation` (App Router) has: `push(href)`, `replace(href)`, `back()`, `forward()`, `refresh()` (re-fetches Server Component data). `redirect(url)` in Server Components/server actions for server-side redirects â€” throws internally, don\'t wrap in try/catch. `permanentRedirect` for 308 redirects. `notFound()` renders the `not-found.tsx` file. `<Link scroll={false}>` prevents scroll-to-top on navigation. `useSelectedLayoutSegment()` â€” active segment for sidebar highlighting.',
    whatBreaks: 'Using `useRouter` from `next/router` (Pages Router) in the App Router â€” different API, will error. Calling `redirect()` inside a try/catch â€” it throws internally.',
    efficientWay: {
    ...
    commonMistakes: ['Importing `useRouter` from `next/router` in App Router â€” use `next/navigation`.', 'Using `<a href>` instead of `<Link>` â€” full page reload.', 'Calling `redirect()` inside try/catch â€” it throws, catch blocks suppress it.'],
    seniorNotes: '`<Link prefetch>` in production is one of Next.js\'s biggest DX wins â€” users experience near-instant navigations because the next page\'s data is already fetched. In development, prefetching is disabled to avoid excessive network calls.',
...
function NavBar() {
  ...
  return (
    <nav>
      {/* Link â€” prefetches on hover in production */}
      <Link href="/dashboard"
...

  {
    ...
    explanation: '`loading.tsx` in a route segment automatically wraps the `page.tsx` in a Suspense boundary. Shows while Server Component data fetches. `error.tsx` is an error boundary for that segment â€” shows on runtime errors. `not-found.tsx` shows when you call `notFound()`.',
    technicalDeep: '`loading.tsx` exports a default component â€” Next.js wraps `<Suspense fallback={<Loading />}><Page /></Suspense>` automatically. The loading UI renders immediately, page content streams in. This enables instant route transitions with skeleton screens. `error.tsx` must include `"use client"` â€” it receives `error: Error` and `reset: () => void` props. `reset()` re-renders the error boundary content (retries the Server Component). Global error: `app/global-error.tsx` catches root layout errors. Segment-level `error.tsx` catches that segment without affecting the rest of the page. `not-found.tsx`: `notFound()` in Server Components triggers this â€” use for missing records. Route-level `not-found.tsx` shows for that route; root `app/not-found.tsx` is the global 404.',
    whatBreaks: 'Missing `"use client"` on `error.tsx` â€” Next.js requires it to be a Client Component. Returning a Promise from `loading.tsx` â€” it must be a synchronous component.',
    efficientWay: {
      title: 'Loading and error patterns',
      approaches: [
        { name: 'loading.tsx for every data-heavy route segment', verdict: 'best', reason: 'Instant perceived navigation â€” users see a skeleton while data loads.' },
        ...
        { name: 'Skeleton UI matching the actual layout', verdict: 'best', reason: 'Reduces CLS (layout shift) â€” skeleton same size as real content.' }
      ],
    ...
    commonMistakes: ['error.tsx without "use client" â€” will not work.', 'loading.tsx that doesn\'t match the page layout size â€” causes layout shift.', 'Not calling notFound() when a record doesn\'t exist â€” showing an empty page instead.'],
    seniorNotes: '`loading.tsx` + streaming is Next.js\'s implementation of progressive HTML: the shell (nav, layout) is sent to the browser immediately, content slots stream in as they resolve. This is better than the old "wait for all data, then render" model â€” users see content faster.',
    ...
    codeExamples: [{ lang: 'tsx', label: 'loading.tsx, error.tsx, not-found.tsx', code: `// app/dashboard/loading.tsx â€” instant skeleton while data fetches
...
// app/dashboard/error.tsx â€” catches fetch/render errors
'use client';
...
export default function UserNotFound() {
...

  {
    ...
    analogy: 'Next.js Image is a smart photographer\'s assistant â€” they deliver the perfect size print for each frame, never more, never less.',
    explanation: '`next/image` serves images in optimal formats (WebP/AVIF), resizes for the requesting device, prevents CLS (you must provide `width`/`height` or `fill`), and lazy-loads by default. `next/font` downloads Google Fonts at build time â€” zero CLS, no external request.',
    technicalDeep: 'Image component: `<Image src="/photo.jpg" alt="desc" width={800} height={600} />`. For dynamic size (fill the parent): `<div style={{ position: "relative" }}><Image src={url} fill alt="..." className="object-cover" /></div>`. `priority` prop on above-the-fold images â€” disables lazy loading, preloads. Remote images: add the domain to `next.config.js` `images.remotePatterns`. The optimization happens in an edge function â€” Vercel handles it; self-hosted needs a custom loader or `sharp`. Font: `import { Inter } from "next/font/google"; const inter = Inter({ subsets: ["latin"] })`. Apply: `className={inter.className}` or as a CSS variable. Fonts are downloaded at build time and served self-hosted â€” no Flash of Unstyled Text (FOUT), no layout shift (CLS = 0 for text). Local fonts: `next/font/local` with `src` pointing to the font file.',
    whatBreaks: 'No `width`/`height` or `fill` on `<Image>` â€” CLS error. Remote images from unconfigured domains â€” blocked by default. `<img>` tags bypassing next/image â€” no optimization, potential CLS.',
    efficientWay: {
      ...
      approaches: [
        ...
        { name: 'Use <img> and link to Google Fonts directly', verdict: 'weak', reason: 'Manual optimization, CLS, and performance regressions â€” defeats the purpose of Next.js.' }
      ],
    ...
    commonMistakes: ['Missing `width`/`height` â€” Next.js throws.', 'Not adding `priority` to the hero image â€” LCP score suffers.', 'Remote images without adding the domain to remotePatterns.'],
    ...
    codeExamples: [{ lang: 'tsx', label: 'Optimized image and font setup', code: `// app/layout.tsx â€” font setup
...
function HeroSection() {
  return (
    <section>
      {/* Above-fold image â€” priority prevents lazy loading */}
  ...
  /* â”€â”€ PHASE 7: Next.js Data & Rendering â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
  {
    ...
    eli5: 'In Next.js Server Components, you use `fetch()` directly â€” no useEffect needed. Next.js caches the response automatically so the same request isn\'t repeated on every page view.',
    analogy: 'Fetching in Server Components is like a chef making stock once and keeping it on the stove â€” everyone who needs it gets it from the pot, not from a fresh batch.',
    explanation: 'In Server Components, `fetch()` is enhanced by Next.js â€” it caches, deduplicates, and supports revalidation. Fetch in the component that needs the data (no prop drilling). Multiple fetches for the same URL in one request are deduplicated automatically.',
    technicalDeep: 'Next.js extends the native `fetch`: `cache` option controls caching behavior. `fetch(url, { cache: "force-cache" })` â€” static, cache forever (default for SSG). `fetch(url, { cache: "no-store" })` â€” dynamic, never cache (SSR). `fetch(url, { next: { revalidate: 60 } })` â€” ISR, revalidate after 60 seconds. In Next.js 15, `cache: "no-store"` is the new default (opt-in to caching). Deduplication: same URL + same options fetched by two components in one render cycle â†’ one request. Data fetching waterfall: if component A awaits before rendering B which also fetches, they run sequentially. Fix: use `Promise.all` for parallel fetches, or fetch at the page level and pass data down. `cache()` function from React for memoizing arbitrary async functions across a request. `unstable_cache` from `next/cache` for cross-request caching of non-fetch async functions (e.g., database queries).',
    whatBreaks: 'Fetching in a loop (fetch per item) â†’ N sequential requests. Use `Promise.all(items.map(id => fetch(...)))` for parallel. Forgetting `{ cache: "no-store" }` on user-specific data â€” shared cached response across users.',
    efficientWay: {
      title: 'Data fetching patterns',
      approaches: [
        { name: 'Fetch in the Server Component that uses the data', verdict: 'best', reason: 'Colocation â€” the data and its rendering are in the same file.' },
    ...
    commonMistakes: ['User-specific data without `no-store` â€” shared cache across users, security issue.', 'Sequential awaits for independent data â€” use Promise.all.', 'Fetching in useEffect when a Server Component could do it without client JS.'],
    seniorNotes: '`unstable_cache` is the Server Component equivalent of React Query\'s `staleTime` for database queries â€” caches the result across requests and revalidates on a schedule or on-demand. It\'s the key primitive for making database-backed pages fast.',
    ...
    codeExamples: [{ lang: 'tsx', label: 'Fetch patterns in Server Components', code: `// app/dashboard/page.tsx â€” parallel fetches
async function DashboardPage() {
  // âœ… Parallel â€” both start at the same time
...
// Called in multiple components â€” runs DB query once per request

// unstable_cache â€” persists across requests
...
const getCachedConfig = unstable_cache(
...

  {
    ...
    eli5: 'Next.js can build pages ahead of time (SSG â€” super fast), build them fresh for each request (SSR â€” always up to date), rebuild them occasionally (ISR â€” best of both), or stream parts of the page as data arrives.',
    analogy: 'SSG is a printed brochure â€” made once, given to everyone. SSR is a custom letter â€” written each time. ISR is a brochure that reprints itself every hour. Streaming is handing pages out as the printer prints them.',
    explanation: 'SSG: `fetch(url, { cache: "force-cache" })` or no fetch â€” page built at build time. SSR: `fetch(url, { cache: "no-store" })` â€” rendered per request. ISR: `{ next: { revalidate: N } }` â€” static HTML rebuilt after N seconds. Streaming: multiple `<Suspense>` boundaries let Next.js send HTML in chunks.',
    technicalDeep: 'The rendering strategy is inferred from the fetch options in a route segment. Any `no-store` fetch makes the route dynamic (SSR). All `force-cache` or revalidated fetches â†’ static. `generateStaticParams()` in dynamic routes: `export async function generateStaticParams() { const posts = await getPosts(); return posts.map(p => ({ slug: p.slug })); }` â€” pre-generates pages for each slug at build time. ISR on-demand revalidation: `revalidatePath("/blog/my-post")` or `revalidateTag("posts")` â€” called from a Server Action or API route to immediately invalidate cache. Streaming: wrap slow Server Components in `<Suspense>` â€” faster data resolves first, slow ones stream in. `loading.tsx` is the route-level Suspense fallback. Nested Suspense for granular streaming.',
    whatBreaks: 'Dynamic route without `generateStaticParams()` â€” 404 at build time for static export. Using `revalidatePath` in a Client Component â€” it only works on the server.',
    efficientWay: {
      title: 'Choosing a rendering strategy',
      approaches: [
        { name: 'Default to ISR â€” revalidate every 60-3600 seconds', verdict: 'best', reason: 'Static performance + fresh data after a short delay. Best of both worlds for most content.' },
        ...
        { name: 'SSG for marketing/docs pages', verdict: 'best', reason: 'Fastest possible load â€” HTML is pre-built and served from CDN edge.' }
      ],
    ...
    commonMistakes: ['No-store on public pages â€” SSR overhead for content that could be static.', 'force-cache on user-specific pages â€” serving one user\'s data to another.', 'Missing generateStaticParams for dynamic SSG routes.'],
    seniorNotes: 'On-demand ISR (`revalidatePath` / `revalidateTag`) is the production-grade pattern: pages stay static until a content change triggers a revalidation. CMS webhooks call a Next.js API route that calls `revalidatePath` â€” the next request gets the freshly built page.',
    ...
    codeExamples: [{ lang: 'tsx', label: 'Rendering strategies in one app', code: `// SSG â€” blog post, built at build time
...
// SSR â€” user dashboard
async function DashboardPage() {
  const user = await fetch('/api/me', { cache: 'no-store' }).then(r => r.json()); // â† SSR
  return <Dashboard user={user} />;
}

// ISR + streaming â€” product page with streamed reviews
async function ProductPage({ params }: { params: { id: string } }) {
  const product = await fetch(\`/api/products/\${params.id}\`, {
    next: { revalidate: 300 } // ISR â€” fresh every 5 min
  }).then(r => r.json());
  return (
    <div>
...

  {
    ...
    eli5: 'Route Handlers are server-side functions that respond to HTTP requests. They live inside your Next.js app and act as a mini API server â€” perfect for handling forms, webhooks, and authenticated requests.',
    ...
    explanation: 'Create `app/api/[path]/route.ts` â€” export named functions for HTTP methods: `GET`, `POST`, `PUT`, `DELETE`, `PATCH`. They receive a `Request` object and return a `Response`. Used for: webhooks, file uploads, authenticated API calls, third-party service proxies.',
    ...
    whatBreaks: 'Naming the file `api.ts` instead of `route.ts` â€” Next.js won\'t recognize it. Not returning a Response â€” handler returns undefined, Next.js sends 500. Using `GET` and `POST` in the same export â€” separate named function exports.',
    efficientWay: {
      title: 'API routes',
      approaches: [
        ...
        { name: 'Server Actions instead of POST route handlers for form mutations', verdict: 'best', reason: 'Server Actions are simpler â€” no fetch in the client, works with progressive enhancement.' },
    ...
    commonMistakes: ['File named api.ts instead of route.ts.', 'Forgetting to return a Response â€” causes 500.', 'Not validating the request body before processing â€” add Zod parsing.'],
    seniorNotes: 'Never call your own Next.js Route Handlers from Server Components or Server Actions â€” call the underlying service (database, external API) directly. Internal HTTP round-trips add latency and bypass all server-side optimization.',
...
export async function POST(request: NextRequest) {
...

  {
    ...
    eli5: 'Middleware is code that runs before every request reaches your page â€” like a security guard at the entrance. You can check ID (auth token), send people somewhere else (redirect), or stamp their badge (set headers) before they even get in.',
    analogy: 'Middleware is the airport security checkpoint â€” every passenger (request) goes through it before boarding (reaching the page). You can let them through, redirect to another gate, or stamp their passport (set headers).',
    explanation: 'Next.js Middleware runs at the Edge (globally, before the page loads) and can inspect every incoming request â€” checking auth cookies, redirecting, rewriting URLs, or setting response headers. It runs before any page renders, making it ideal for auth guards, A/B testing, and geolocation routing.',
    technicalDeep: 'File location: `middleware.ts` at the project root. `export function middleware(request: NextRequest) { ... }`. Runs in Edge Runtime â€” no Node.js APIs, no file system, no database calls. `NextResponse.redirect(new URL("/login", request.url))` â€” hard redirect. `NextResponse.rewrite(new URL("/us/dashboard", request.url))` â€” serve different content at same URL. `NextResponse.next()` â€” continue to the page. `matcher` config: `export const config = { matcher: ["/dashboard/:path*"] }` â€” critical for performance, skips static assets. Auth pattern: read cookie with `request.cookies.get("session")?.value`, verify JWT using `jose` (NOT `jsonwebtoken` â€” Node.js only). Geolocation: `request.geo?.country`. A/B testing: rewrite to different page based on a cookie value. Header injection: `const res = NextResponse.next(); res.headers.set("x-custom", "value"); return res`. Important: Middleware is a fast first-line guard, NOT a replacement for server-side auth checks â€” still verify auth in Server Components for sensitive data.',
    whatBreaks: 'Using Node.js APIs in middleware â€” it runs in Edge Runtime. Forgetting `matcher` â€” middleware runs on every `/_next/static/` request too, causing unnecessary overhead. Doing database queries in middleware â€” it\'s on the hot path of every single request.',
    efficientWay: {
      title: 'Middleware use cases',
      approaches: [
        { name: 'Auth redirect guard', verdict: 'best', reason: 'Redirect to login before the page even loads â€” no flash of protected content.' },
        ...
        { name: 'Complex business logic in middleware', verdict: 'weak', reason: 'Middleware is fast and lightweight by design â€” heavy logic belongs in route handlers or server actions.' }
      ],
    ...
    commonMistakes: [
      'Forgetting `matcher` â€” middleware then runs on static files too.',
      'Using Node.js-only packages like `jsonwebtoken` â€” use `jose` for JWT at edge.',
      'Doing DB calls in middleware â€” too slow for every request; use an edge KV store (Upstash) if caching is needed.'
    ],
    seniorNotes: 'Middleware runs in V8 Isolates at Vercel\'s global edge â€” ~1ms latency worldwide. This is why it\'s ideal for auth checks: the user is redirected before a backend even receives the request. The architectural rule: "middleware decides WHERE to route, server components decide WHAT to show." Never mix these concerns.',
    interviewQuestions: [
    ...
    codeExamples: [
      {
        ...
        code: `// middleware.ts (project root â€” NOT inside /app)
import { NextRequest, NextResponse } from 'next/server';
...
export async function middleware(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.next(); // verified â€” continue
  } catch {
...
export const config = {
...

  {
    ...
    eli5: 'Server Actions are functions that run on the server but can be called from a form or button in a Client Component â€” without you writing an API route. Next.js handles the network call automatically.',
    ...
    explanation: 'Server Actions are async functions marked with `"use server"`. They can be called from `<form action={action}>` or from Client Components as regular async functions. On call, Next.js POSTs to the server, runs the function, and returns the result â€” no manual fetch, no API route.',
    technicalDeep: '`"use server"` directive at the top of a function (inside a Server Component) or a file. Inline: `async function create(formData: FormData) { "use server"; ... }`. Separate file: add `"use server"` at the top, all exports are server actions. Form integration: `<form action={create}>` â€” works without JS (progressive enhancement). Client Component call: `<button onClick={() => create(data)}>` â€” or `useTransition` + `action` for pending state. `useFormState` (App Router) and `useFormStatus` track state. Validation: parse `formData.get("name")` or use Zod. Revalidation: call `revalidatePath("/users")` inside the action to update cached pages. `redirect()` inside an action navigates the user after success. Security: actions are POST endpoints â€” validate input (Zod), check auth, use CSRF protection (Next.js adds it automatically for same-origin forms).',
    whatBreaks: 'Returning non-serializable values from a Server Action â€” only JSON-serializable types cross the boundary. Calling `redirect()` inside a try/catch â€” it throws and the catch suppresses it.',
    efficientWay: {
      title: 'Server Actions',
      approaches: [
        ...
        { name: 'Route Handlers for mutations from non-form sources (webhooks, scripts)', verdict: 'best', reason: 'Route Handlers are HTTP endpoints â€” any client can call them.' },
    ...
    commonMistakes: ['`redirect()` inside try/catch â€” catch block suppresses the throw redirect needs.', 'Not validating form data with Zod before using it.', 'Forgetting `revalidatePath` after a mutation â€” UI shows stale data.'],
    seniorNotes: 'Server Actions with `useOptimistic` enable optimistic UI updates that feel instant â€” update the UI immediately, revert if the action fails. This pattern matches React Query\'s optimistic update capability without client-side state management.',
...
export function CreateUserForm() {
...

  {
    ...
    analogy: 'Node.js runtime is a full kitchen with every appliance â€” but only in one city. Edge runtime is a tiny kitchenette in every city in the world â€” instant service, but no oven, no heavy appliances.',
    explanation: 'Pages, route handlers, and middleware can declare which runtime they use. Node.js runtime has full access to all Node APIs, database clients, and the file system. Edge runtime is a V8 Isolate (no Node.js) â€” it starts faster, runs globally, and is cheaper, but can only use Web Platform APIs.',
    technicalDeep: '`export const runtime = "nodejs"` (default) or `export const runtime = "edge"`. Node.js runtime: full Node.js APIs, can connect to databases (Prisma, pg, mongoose), file system, native modules. Runs as a Serverless Function in one region. Cold starts: ~200ms-1s. Edge runtime: Web APIs only (fetch, crypto, URL, Headers, TextEncoder). No Node.js builtins (no `Buffer`, no `fs`, no `path`). Runs in V8 Isolates globally at all edge locations. Cold starts: ~0ms (always warm). Use cases â€” Edge: middleware, auth checks, A/B tests, geolocation routing, streaming responses. Node.js: database queries, file processing, libraries that require Node APIs. Choosing: default to Node.js, opt into Edge when latency matters AND you can write edge-compatible code. ORM compatibility: Prisma requires Node.js. Edge-compatible alternatives: Drizzle (with SQLite/D1/Turso), Kysely. Database via HTTP: PlanetScale/Neon/Turso all offer HTTP APIs usable at the edge.',
    whatBreaks: 'Importing `jsonwebtoken`, `bcrypt`, or `prisma` in an Edge route â€” these use Node.js APIs and throw at build or runtime. Not specifying runtime on a route that imports edge-incompatible packages.',
    efficientWay: {
    ...
    commonMistakes: [
      'Importing Node.js packages in Edge routes â€” error at runtime or build time.',
      'Using Edge everywhere just because it sounds faster â€” the latency win disappears if you still hit a single-region DB.',
      'Not checking ORM/package compatibility before choosing Edge runtime.'
    ],
    seniorNotes: 'The Edge vs Node choice reflects a deeper architectural question: "where does the data live?" If your DB is in one region, making the API globally edge is pointless â€” the DB roundtrip still adds 200ms. Edge wins when the computation itself (auth check, redirect decision, personalization) is the bottleneck, and data lives in a global edge KV store (Upstash, Cloudflare KV) or isn\'t needed at all.',
    interviewQuestions: [
    ...
    codeExamples: [
      {
        ...
        code: `// app/api/geo/route.ts â€” geolocation, no DB needed â†’ Edge
...
// app/api/users/route.ts â€” database query â†’ Node.js (default)
export const runtime = 'nodejs'; // explicit for clarity

import { db } from '@/lib/db'; // Prisma â€” requires Node.js
export async function GET() {
  ...
  /* â”€â”€ PHASE 8: Auth, Database & Production â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
  {
    ...
    analogy: 'NextAuth is a complete doorbell system â€” camera, lock, intercom, and visitor log. You plug it in and it handles who gets in, who doesn\'t, and keeps a record.',
    ...
    technicalDeep: 'Setup: `npm i next-auth@beta`. Create `auth.ts`: `import NextAuth from "next-auth"; import GitHub from "next-auth/providers/github"; export const { auth, handlers, signIn, signOut } = NextAuth({ providers: [GitHub] })`. Route handler: `app/api/auth/[...nextauth]/route.ts` â€” `export const { GET, POST } = handlers`. Session in Server Components: `const session = await auth()`. Middleware protection: `middleware.ts` â€” `export { auth as middleware } from "./auth"`. Callbacks: `jwt` callback to add user role to the token; `session` callback to expose it to the client. Database adapter: `@auth/prisma-adapter` â€” stores users, sessions, accounts in your DB. `signIn("github")` from a Client Component triggers OAuth flow. `signOut()` clears the session. `useSession()` from `next-auth/react` for Client Components. Credentials provider: hash passwords with bcrypt, validate in the `authorize` function.',
    whatBreaks: 'Missing `NEXTAUTH_SECRET` env var â€” Auth.js throws. OAuth callbacks blocked by wrong redirect URI in the provider dashboard. Accessing session in Server Components without `await` â€” `auth()` is async.',
    efficientWay: {
      ...
      approaches: [
        ...
        { name: 'Clerk (hosted auth service)', verdict: 'best', reason: 'Zero setup, great DX, handles user management UI â€” paid for production volume.' },
        { name: 'Build auth from scratch (JWT + bcrypt)', verdict: 'ok', reason: 'Good for learning; don\'t do it in production â€” too many security pitfalls.' }
      ],
      recommendation: 'NextAuth for most projects â€” free, self-hosted, widely used. Clerk for projects where user management UI and hosted auth justify the cost.'
    },
    commonMistakes: ['Missing AUTH_SECRET in production.', 'Not protecting routes in middleware â€” auth check in each page is error-prone.', 'Using JWT sessions when database sessions are needed for immediate revocation.'],
...
// middleware.ts â€” protect all dashboard + api routes
export { auth as middleware } from './auth';
export const config = { matcher: ['/dashboard/:path*', '/api/:path*'] };

// Server Component â€” read session
async function Page() {
...
// Client Component â€” sign in button
'use client';
...
<button onClick={() => signOut()}>Sign out</button>` }]
...

  {
    ...
    commonMistakes: ['New PrismaClient per request in serverless â€” connection pool exhausted.', 'Forgetting `npx prisma generate` after schema update.', 'Not running migrations in production â€” `npx prisma migrate deploy` (not `dev`) for production.'],
    seniorNotes: 'Prisma Migrate\'s shadow database pattern: every migration runs against a shadow copy to verify it\'s safe before touching the real database. In production, `migrate deploy` applies pending migrations without the shadow DB â€” suitable for CI/CD.',
...
// lib/db.ts â€” singleton for serverless
...
async function getUser(id: string) {
...

  {
    ...
    explanation: 'Vercel is the natural Next.js host â€” zero-config deployment, automatic edge optimization. The production checklist covers: environment variables, security headers, error monitoring, Core Web Vitals, and bundle size.',
    technicalDeep: 'Vercel deployment: connect GitHub repo â†’ automatic CI/CD on every push. Preview deployments on every PR. Environment variables: add in Vercel dashboard, never commit `.env.local`. Security headers: add in `next.config.js` `headers()` function â€” Content-Security-Policy, X-Frame-Options, X-Content-Type-Options. CSP is the most impactful security header. Bundle analysis: `npm i @next/bundle-analyzer`. `ANALYZE=true npm run build` â€” opens a treemap of chunk sizes. Web Vitals monitoring: `npm i web-vitals` + `export function reportWebVitals(metric)` in layout â€” send to analytics. Sentry: `npm i @sentry/nextjs; npx @sentry/wizard@latest -i nextjs` â€” captures unhandled errors and performance traces. `next.config.js` optimizations: `compress: true` (gzip), `poweredByHeader: false` (remove X-Powered-By), `images.formats: ["image/avif", "image/webp"]`. Rate limiting: use Upstash Redis + `@upstash/ratelimit` in Middleware for API routes.',
    whatBreaks: 'Missing environment variables in Vercel â€” `undefined` in production, hard to debug. Missing `NEXTAUTH_SECRET` â€” auth fails silently. Open CORS headers on all origins â€” security vulnerability.',
    efficientWay: {
      ...
      approaches: [
        ...
        { name: 'Netlify', verdict: 'best', reason: 'Good alternative â€” slightly less Next.js-specific optimization.' },
        { name: 'Self-hosted with Docker', verdict: 'ok', reason: 'More control, more ops work â€” necessary if data residency requirements exist.' }
      ],
      recommendation: 'Deploy to Vercel for every project. Run Lighthouse in CI (`lighthouse-ci`) to catch performance regressions before they ship. Add Sentry on day one â€” production errors are invisible without it.'
    },
    commonMistakes: ['Not setting environment variables in Vercel dashboard.', 'Committing .env.local to git â€” use .gitignore and check with `git status`.', 'No error monitoring â€” bugs in production are invisible without Sentry.'],
    seniorNotes: 'Vercel\'s Edge Middleware (running in V8 Isolates globally) executes in ~1ms worldwide â€” it\'s where auth checks, A/B testing, and geo-routing should live. Database queries must never run in Edge Middleware â€” use it for lightweight decisions that route to the right regional compute.',
