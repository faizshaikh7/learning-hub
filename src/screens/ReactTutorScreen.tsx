      ...
      {topic.interviewQuestions.length > 0 && (
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-sm font-semibold text-orange-400">Interview Questions</span>
          </div>
          <ul className="space-y-2">
            {topic.interviewQuestions.map((q, i) => (
              <li key={i} className="flex items-start gap-2 text-sm text-gray-300">
                <span className="shrink-0 text-orange-500 font-mono text-xs mt-1">Q{i + 1}.</span>
                {q}
              </li>
            ))}
          </ul>
