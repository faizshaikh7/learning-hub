...
import { MOBILE_CURRICULUM } from '@/data/mobile/curriculum'
import { MOBILE_FLASHCARDS } from '@/data/mobile/flashcards'
import { MOBILE_CASE_STUDIES } from '@/data/mobile/case-studies'
import { MOBILE_LIFECYCLES } from '@/data/mobile/lifecycles'
import { MOBILE_INTERVIEW } from '@/data/mobile/interview'
import { MOBILE_COMPARISONS } from '@/data/mobile/comparisons'
import { MOBILE_CHEATSHEETS } from '@/data/mobile/cheatsheets'
import { MOBILE_CONCEPT_MAP } from '@/data/mobile/concept-map'
import { MOBILE_PROJECTS } from '@/data/mobile/projects'
...
function FlashcardView({ topic }: { topic: CurriculumTopic }) {
  const cards = useMemo(() => MOBILE_FLASHCARDS[topic.id] ?? [], [topic.id])
...
function NotesView({ topic }: { topic: CurriculumTopic }) {
  ...
  const existing = getNoteForTopic(topic.id, 'mobile')
  ...
  useEffect(() => {
    const note = getNoteForTopic(topic.id, 'mobile')
  ...
  const handleChange = useCallback((val: string) => {
    ...
    saveTimer.current = setTimeout(() => {
      upsertNote(topic.id, 'mobile', val)
      setSaved(true)
    }, 800)
  }, [topic.id, upsertNote])
...
function RoadmapView({
  ...
  const phases = useMemo(() => groupByPhase(MOBILE_CURRICULUM), [])
  const completed = Object.values(progress).filter(s => s === 'completed').length
  const total = MOBILE_CURRICULUM.length
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
        <div className="flex items-center justify-between mb-3">
          <div>
            <p className="text-sm font-semibold text-white">Mobile Development Progress</p>
...
/**
 * MobileTutorScreen — full learning experience for the Mobile Development track.
 * Features: lesson viewer, quiz, flashcards, notes, roadmap, and Pomodoro timer.
 */
export default function MobileTutorScreen() {
  ...
  const rawProgress = useMemo(
    () => getProgress('mb') as Record<string, TopicStatus>,
  ...
  const initialTopic = useMemo(() => {
    return (
      MOBILE_CURRICULUM.find(t => (rawProgress[t.id] ?? 'not-started') !== 'completed') ??
      MOBILE_CURRICULUM[0]
    )
  }, [rawProgress])
  ...
  const phaseGroups = useMemo(() => groupByPhase(MOBILE_CURRICULUM), [])

  // Prev/next for bottom navigation
  const currentTopicIdx = useMemo(
    () => MOBILE_CURRICULUM.findIndex(t => t.id === activeTopic.id),
    [activeTopic.id],
  )
  const prevTopic = currentTopicIdx > 0 ? MOBILE_CURRICULUM[currentTopicIdx - 1] : null
  const nextTopic = currentTopicIdx < MOBILE_CURRICULUM.length - 1 ? MOBILE_CURRICULUM[currentTopicIdx + 1] : null
  ...
  const handleSelectTopic = useCallback((t: CurriculumTopic) => {
    ...
    if ((progress[t.id] ?? 'not-started') === 'not-started') {
      markTopic('mobile', t.id, 'in-progress')
  ...
  const handleMarkComplete = useCallback(() => {
    ...
    markTopic('mobile', activeTopic.id, 'completed')
    addXP(10)
    setProgress(p => ({ ...p, [activeTopic.id]: 'completed' }))

    // Auto-advance to next incomplete topic
    const currentIdx = MOBILE_CURRICULUM.findIndex(t => t.id === activeTopic.id)
    const nextTopic = MOBILE_CURRICULUM.slice(currentIdx + 1).find(
  ...
  const totalCount = MOBILE_CURRICULUM.length
