    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
        ...
        <div className="bg-cyan-500/10 border border-cyan-500/20 rounded-lg p-3">
          <p className="text-sm text-cyan-200">{topic.efficientWay.recommendation}</p>
        </div>
      </div>
      ...
      {topic.interviewQuestions.length > 0 && (
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-sm font-semibold text-cyan-400">Interview Questions</span>
          </div>
          <ul className="space-y-2">
            {topic.interviewQuestions.map((q, i) => (
              <li key={i} className="flex items-start gap-2 text-sm text-gray-300">
                <span className="shrink-0 text-cyan-500 font-mono text-xs mt-1">Q{i + 1}.</span>
                {q}
              </li>
            ))}
          </ul>
...
function FlashcardView({ topic }: { topic: CurriculumTopic }) {
  const cards = useMemo(() => FLUTTER_FLASHCARDS[topic.id] ?? [], [topic.id])
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <button
        ...
        className="w-full min-h-48 bg-gray-900 border border-gray-800 rounded-xl p-6 text-left hover:border-cyan-500/40 transition-colors"
      >
        <div className="flex items-center gap-2 mb-4">
          {flipped ? (
            ...
            <span className="text-xs font-semibold text-cyan-400 px-2 py-0.5 bg-cyan-500/10 border border-cyan-500/20 rounded">
              QUESTION
            </span>
          )}
...
function NotesView({ topic }: { topic: CurriculumTopic }) {
  ...
  const existing = getNoteForTopic(topic.id, 'flutter')
  ...
  useEffect(() => {
    const note = getNoteForTopic(topic.id, 'flutter')
  ...
  const handleChange = useCallback((val: string) => {
    ...
    saveTimer.current = setTimeout(() => {
      upsertNote(topic.id, 'flutter', val)
      setSaved(true)
    }, 800)
  }, [topic.id, upsertNote])
  ...
  return (
    <div className="space-y-4 pb-12">
      ...
      <textarea
        ...
        className="w-full min-h-[400px] bg-gray-900 border border-gray-800 rounded-xl p-4 text-gray-300 text-sm leading-relaxed resize-y focus:outline-none focus:border-cyan-500/50 placeholder:text-gray-600 font-mono"
      />
...
function RoadmapView({
  ...
  const phases = useMemo(() => groupByPhase(FLUTTER_CURRICULUM), [])
  const completed = Object.values(progress).filter(s => s === 'completed').length
  const total = FLUTTER_CURRICULUM.length
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
        <div className="flex items-center justify-between mb-3">
          <div>
            <p className="text-sm font-semibold text-white">Flutter Progress</p>
        ...
        <div className="h-2 bg-gray-800 rounded-full">
          <div
            className="h-2 bg-gradient-to-r from-cyan-500 to-cyan-400 rounded-full transition-all duration-500"
            style={{ width: `${pct}%` }}
          />
        </div>
...
function TimerView() {
  ...
  const modeColor = mode === 'focus' ? 'text-cyan-400' : mode === 'short-break' ? 'text-green-400' : 'text-teal-400'
  const ringColor = mode === 'focus' ? 'stroke-cyan-500' : mode === 'short-break' ? 'stroke-green-500' : 'stroke-teal-500'
    ...
    <div className="flex flex-col items-center pb-12 space-y-8">
      ...
      <div className="flex gap-3">
        ...
        <button
          ...
          className="px-8 py-3 rounded-full bg-cyan-500 hover:bg-cyan-600 text-white font-semibold text-lg transition-colors flex items-center gap-2"
        >
...
/**
 * FlutterTutorScreen — full learning experience for the Flutter/Dart track.
 * Features: lesson viewer, flashcards, notes, roadmap, and Pomodoro timer.
 */
export default function FlutterTutorScreen() {
  ...
  const rawProgress = useMemo(
    () => getProgress('ft') as Record<string, TopicStatus>,
  ...
  const initialTopic = useMemo(() => {
    return (
      FLUTTER_CURRICULUM.find(t => (rawProgress[t.id] ?? 'not-started') !== 'completed') ??
      FLUTTER_CURRICULUM[0]
    )
  }, [rawProgress])
  ...
  const currentTopicIdx = useMemo(
    () => FLUTTER_CURRICULUM.findIndex(t => t.id === activeTopic.id),
    [activeTopic.id],
  )
  const prevTopic = currentTopicIdx > 0 ? FLUTTER_CURRICULUM[currentTopicIdx - 1] : null
  const nextTopic = currentTopicIdx < FLUTTER_CURRICULUM.length - 1 ? FLUTTER_CURRICULUM[currentTopicIdx + 1] : null
  ...
  const handleSelectTopic = useCallback((t: CurriculumTopic) => {
    ...
    if ((progress[t.id] ?? 'not-started') === 'not-started') {
      markTopic('flutter', t.id, 'in-progress')
  ...
  const handleMarkComplete = useCallback(() => {
    ...
    markTopic('flutter', activeTopic.id, 'completed')
    addXP(10)
    setProgress(p => ({ ...p, [activeTopic.id]: 'completed' }))

    // Auto-advance to next incomplete topic
    const currentIdx = FLUTTER_CURRICULUM.findIndex(t => t.id === activeTopic.id)
    const nextTopic = FLUTTER_CURRICULUM.slice(currentIdx + 1).find(
  ...
  const totalCount = FLUTTER_CURRICULUM.length
      ...
      >
        ...
        <div className="px-4 py-4 border-b border-gray-800">
          ...
          <div className="flex items-center gap-2 mb-2">
            ...
            <div>
              <p className="text-sm font-semibold text-white">Flutter</p>
